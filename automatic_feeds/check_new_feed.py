...
def format_dict_to_str(coin_item: dict) -> str:
    newdict = {
        ...
        "price_change_percentage_24h": coin_item["data"]["price_change_percentage_24h"]["usd"],
        "thumbnail": coin_item["thumb"],
        "description": coin_item["data"]["content"]["description"] if coin_item["data"]["content"] else "",
        "total_volume": coin_item["data"]["total_volume"]
    }
    return '\n'.join("{}: {}".format(k, v) for k, v in newdict.items())
...
if __name__ == "__main__":
    ...
    with open("issues.md", "w") as f:
        ...
        for coin in trending["coins"]:
            if coin["item"]["market_cap_rank"] < 100 and coin["item"]["symbol"] not in CURRENT_FEEDS:
                item_data = coin["item"]
