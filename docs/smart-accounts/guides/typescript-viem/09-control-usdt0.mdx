...
This guide shows how to control [USDT0](https://docs.usdt0.to/) held by a Flare [personal account](/smart-accounts/overview) from an XRPL wallet.
You will read the account's USDT0 balance, transfer USDT0 to another EVM address, and swap USDT0 for FXRP on [SparkDEX](https://sparkdex.ai/) — all through fee-only [`0xFE` custom instructions](/smart-accounts/custom-instruction).
...
Before running the mutating scripts:

1. Derive your personal account address with the [State Lookup guide](/smart-accounts/guides/typescript-viem/state-lookup-ts#personal-account-of-an-xrpl-address) (or the read-only `balance.ts` script below).
2. Fund that EVM address with FLR and USDT0 from the [Coston2 faucet](https://faucet.flare.network/coston2).
3. Configure `.env` in [flare-viem-starter](https://github.com/flare-foundation/flare-viem-starter) with `XRPL_TESTNET_RPC_URL`, `XRPL_SEED`, and `PRIVATE_KEY` (executor wallet for the Flare submission step).
...
```mermaid
sequenceDiagram
  participant XRPL as XRPL_wallet
  participant PA as PersonalAccount
  participant USDT0 as USDT0_ERC20
  participant Router as SparkDEX_Router
  participant Exec as Executor

  Note over XRPL,Exec: Fee-only 0xFE (netMintAmountXrp equals 0)
  XRPL->>XRPL: Payment with 42-byte memo hash
...
Both mutating scripts follow the same three-step `0xFE` protocol used elsewhere in the starter:
...
`transfer.ts` builds a single-call UserOp that invokes `USDT0.transfer(recipient, amount)` from the personal account.
...
`DEFAULT_TRANSFER_RECIPIENT` in `config.ts` is a hardcoded demo address — change it before sending real funds.
...
`swap-usdt0-to-fxrp.ts` swaps USDT0 for FXRP on SparkDEX from the personal account.
...
`amountOutMinimum` is derived from `DEFAULT_AMOUNT_OUT_MINIMUM_UNITS` (`0.3` FXRP in the demo).
...
<details>
  <summary>Expected output (transfer)</summary>
