---
sidebar_position: 3
slug: raw-custom-instruction-ts
...
import RawCustomInstructionsScript from "!!raw-loader!/examples/developer-hub-javascript/smart-accounts/custom-instructions-raw.ts";

The [Raw Custom Instruction overview](/smart-accounts/raw-custom-instruction) explains how Flare smart accounts replay an [EIP-4337](https://eips.ethereum.org/EIPS/eip-4337) `PackedUserOperation` carried in full in an XRPL `Payment` memo (opcode `0xFF`).
...
Use the raw flow when you do not want to operate or coordinate with an executor and your call batch fits inside the XRPL memo cap.
...
<CodeBlock language="typescript" title="src/custom-instructions-raw.ts">
...
- Explore the [Raw Custom Instruction overview](/smart-accounts/raw-custom-instruction).
