...
The script in this guide hits the cap head-on: it executes three calls on three example smart contracts, but the XRPL memo field is capped at `1024` bytes, and the three calls together exceed that budget when ABI-encoded.
...
The [Hash Custom Instruction guide](/smart-accounts/guides/typescript-viem/hash-custom-instruction-ts) shows the 0xFE variant that ships only `keccak256(userOp)` in the memo and lifts both limitations; pick that flow when your batches overflow the cap, when individual calls cannot be encoded, or when you want call payloads to stay off the public XRPL ledger.
