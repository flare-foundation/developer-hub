...
The first payment is the one that actually mints FXRP: the XRP arrives at the core vault on XRPL, the FAssets executor proves the direct mint payment to Flare, and the `MasterAccountController` mints FXRP into the personal account and executes the embedded approve in the same on-chain transaction.
