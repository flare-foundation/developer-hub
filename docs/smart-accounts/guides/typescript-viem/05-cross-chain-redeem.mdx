...
A prerequisite for the script to work is a Sepolia EOA that already holds FXRP and enough SETH to cover the LayerZero native fee.
...
## Waiting for `RedemptionRequested`

A successful flow results in the `AssetManager` on Flare emitting a `RedemptionRequested` event with the composer as the `redeemer` indexed argument.
...
The actual XRP payment to `underlyingAddress` is handled off-chain by an F-Asset agent after the redemption request is filed.
