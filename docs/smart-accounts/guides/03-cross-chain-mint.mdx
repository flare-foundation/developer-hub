...
The [State Lookup guide](/smart-accounts/guides/state-lookup-ts#personal-account-of-an-xrpl-address) explains how to obtain the personal account address from the XRPL address.
...
The Flare client is the same one introduced in the [State Lookup guide](/smart-accounts/guides/state-lookup-ts); a second Viem public client is added for Sepolia, plus a wallet client and a signing account for write operations.
...
The `getPersonalAccountAddress` and `getFxrpAddress` functions are covered in the [State Lookup guide](/smart-accounts/guides/state-lookup-ts).
...
- [Cross-Chain Redeem guide](/smart-accounts/guides/cross-chain-redeem-ts)
