...
- `bridge`
  ...
  - `mint-tx`: make a deposit do an agent's vault
...
The bellow-described CLi commands are intended to work in tandem with one another.
...
The `walletId` is a Flare-designated parameter, intended wallet identification by the operator.
...
Deposit `vault` drops of FXRP into the Firelight vault, registered with the `MasterAccountController` contract with the `vaultId`.
The complete list of the registered vaults can be obtained from the `MasterAccountController` contract.
{/* TODO:(Nik) Write a separate guide in which you explain how to do this, and link to it here. */}

:::warning
The registered vaults can be of either the Firelight or the Upshift type, so make sure that you choose the right one for the instruction you are sending.
:::
...
A `value` of drops FXRP is marked to be withdrawn from the Firelight vault, registered with the `MasterAccountController` contract with the `vaultId`, to the user's personal account.
...
{/* TODO */}
When the withdrawal is requested, a period value exposed in the event that is emitted.
...
The collateral reservation and deposit instruction signals two intents at once.
First, to reserve collateral in an agent's vault, registered with the `MasterAccountController` contract with the `agentVaultId`, the necessary collateral to mint `value` lots of FXRP.
And second, to deposit the newly minted FXRP into a Upshift type vault, registered with the `MasterAccountController` contract with the `vaultId`.
...
Deposit `vault` drops of FXRP into the Upshift vault, registered with the `MasterAccountController` contract with the `vaultId`.
The complete list of the registered vaults can be obtained from the `MasterAccountController` contract.
{/* TODO:(Nik) Write a separate guide in which you explain how to do this, and link to it here. */}

:::warning
The registered vaults can be of either the Upshift or the Upshift type, so make sure that you choose the right one for the instruction you are sending.
:::
...
Begin the withdrawal process from an Upshift vault.
A `value` of drops FXRP is marked to be withdrawn from the Upshift vault, registered with the `MasterAccountController` contract with the `vaultId`, to the user's personal account.
...
The `value` parameter is the date of when the redemption was requested.
It should given as an 8-digit number `YYYYMMDD`, where `YYYY` is the year, `MM` the month (from `1` to `12`) and `DD` (from `1` to `31`) the day of the redemption request.
...
The `transactionHash` can be obtain by calling the `bridge` command with the `instruction` positional argument.
...
The `decode` command is an inverse of the `encode` command.
