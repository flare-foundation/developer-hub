...
The [State Lookup guide](/smart-accounts/guides/state-lookup-ts#personal-account-of-an-xrpl-address) explains how we can acquire its address.
...
We make an XRPL Payment transaction to one of the operator's XRPL addresses (the [State Lookup guide](/smart-accounts/guides/state-lookup-ts#operators-xrpl-addresses) explains how we can obtain these).
...
The vehicle for bridging the instruction is the [Flare Data Connector](/fdc/overview), which puts a maximum cap of 180 seconds on the process duration.
