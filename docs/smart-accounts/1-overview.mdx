...
1. The XRPL user sends a `Payment` to the FAssets [Core Vault](/fassets/core-vault) XRPL address that mints FXRP to the smart account (or with a [memo instruction](/smart-accounts/custom-instruction-comparison)), as described in [FAssets minting](/fassets/minting).
...
graph TB
 ...
 CoreVault(["<b>Core Vault</b><br/><i>XRPL address</i>"])
 end
 ...
 User -- "1 - Payment + memo" --> CoreVault
 CoreVault -. "2 - observe payment" .-> Executor
...
<p style={{ textAlign: "center", fontStyle: "italic" }}>
  Direct-minting flow: XRPL `Payment` to the Core Vault carrying a memo (1) →
  executor observes and calls `AssetManager.executeDirectMinting` (2, 3) →
  `AssetManager` mints FXRP and calls back into `handleMintedFAssets` (4) →
  `MasterAccountController` forwards FXRP and dispatches the memo to the user's
  `PersonalAccount` (5).
</p>
...
| ---------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `00`       | `collateralReservation` | _(Legacy CRT)_ Prefer [FAssets minting](/fassets/minting). Reserve a `value` of lots of collateral in the agent vault, registered with the `agentVaultId` with the `MasterAccountController` contract. |
...
| Command ID | Action                            | Description                                                                                                                                                                                                                                                                                                                                                                |
| ---------- | --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `00`       | `collateralReservationAndDeposit` | _(Legacy CRT)_ Prefer [FAssets minting](/fassets/minting) then `deposit`. Reserve a `value` of lots of collateral in the agent vault, registered with the `agentVaultId` with the `MasterAccountController` contract. After successful minting, deposit the FXRP into the Firelight type vault, registered with the `vaultId` with the `MasterAccountController` contract. |
...
When the user mints FXRP directly to their smart account via [FAssets minting](/fassets/minting), the FAssets `AssetManager` calls back into [`handleMintedFAssets`](/smart-accounts/reference/IMasterAccountController#handlemintedfassets) on the `MasterAccountController`.
...
This can be any of the instructions listed above — transferring FXRP to another address, redeeming FXRP, depositing it into a vault, and so on.
Minting FXRP itself uses [FAssets minting](/fassets/minting) (Core Vault payment), not a CRT Smart Accounts instruction.
