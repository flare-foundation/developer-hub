...
The memo instruction surface lets a [`PersonalAccount`](/smart-accounts/reference/IPersonalAccount) execute [custom user operations](/smart-accounts/custom-instruction) embedded in the XRPL `Payment` memo when FAssets are direct-minted into the account.

### `mintedFAssets`

Called by the FAssets `AssetManager` when FXRP is direct-minted into a personal account.
Decodes the XRPL memo, distributes the minted FAssets between the personal account and the executor, and dispatches any embedded user operation.
...
Emitted when a memo-encoded user operation (`0xFF`) executes against a personal account.
