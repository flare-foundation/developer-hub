...
For the [`0xFE` hash flow](/smart-accounts/hash-custom-instruction), the `_data` parameter carries the ABI-encoded `PackedUserOperation` whose hash was committed to in the memo.
...
- `_data`: Extra data not contained in the XRPL memo. For the `0xFE` hash flow this is the ABI-encoded `PackedUserOperation`; for all other instruction IDs it is ignored.
...
Emitted when a memo-dispatched user operation (`0xFF` full flow or `0xFE` hash flow) executes against the personal account.
...
Thrown by the [`0xFE` hash custom instruction](/smart-accounts/hash-custom-instruction) when `keccak256(_data)` does not match the 32-byte hash carried in the XRPL memo.
