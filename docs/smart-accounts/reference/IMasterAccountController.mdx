...
:::caution preview
`reserveCollateral` and `executeDepositAfterMinting` belong to the executor-facing facets of the diamond and are documented here for completeness; cross-check the facet ABI shipped in the periphery release before consuming them — facet splits can shift between versions.
:::
...
Memo instructions allow a [`PersonalAccount`](/smart-accounts/reference/IPersonalAccount) to execute [custom user operations](/smart-accounts/custom-instruction) embedded in the XRPL `Payment` memo when FAssets are direct-minted into the account.
...
See [Custom Instruction](/smart-accounts/custom-instruction) for the end-to-end flow.
...
- `_memoData`: The raw XRPL memo bytes carrying the [memo instruction](/smart-accounts/custom-instruction).
...
Emitted when a memo-encoded user operation (`0xFF`) executes with the personal account.
...
These errors are surfaced by [`mintedFAssets`](#mintedfassets) and the memo dispatch path.
...
Thrown when [`mintedFAssets`](#mintedfassets) is called by any address other than the FAssets `AssetManager`.
...
Thrown when the first byte of the memo is not a recognized memo instruction ID (`0xFF`, `0xE0`, `0xE1`, `0xE2`, `0xD0`, `0xD1`).
