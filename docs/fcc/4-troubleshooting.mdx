...
```bash
source .env
source config/coston2/extension.env
REG=$(jq -r '.[]|select(.name=="FlareTeeManager").address' config/coston2/deployed-addresses.json)
cd tools
go run ./cmd/query-tee -ext "$((EXTENSION_ID))" -reg "$REG" -rpc "$CHAIN_URL"
```

The `-ext` flag takes the **decimal** extension ID — `$((EXTENSION_ID))` converts the hex value from `config/coston2/extension.env`.
...
You can also pass a TEE machine address as a positional argument to inspect that machine.
...
### Related: Name Mismatch Between Solidity And The Extension
...
In the Hello World scaffold those files are `go/internal/config/config.go`, `python/app/config.py`, and `typescript/src/app/config.ts`.
...
## New Reward Epoch Signed Before It Is Live Onchain

### Symptoms

- You restart your local extension proxy and it picks up a new reward epoch, but the proxy does not work correctly afterward.
- The epoch the proxy picked up was initialized and signed on Flare, but is not actually active on chain yet.

### Root Cause: Chain Lags The Signing Policy By One Epoch

A new reward epoch can be initialized and signed roughly two hours before it actually starts on chain.
If you restart the proxy inside that window, it picks up the new epoch even though the chain is still one epoch behind, and the proxy fails because that epoch isn't live yet.

### Solution

Set `initial_signing_policy_offset=1` in the proxy config file (on the scaffold this is `/config/proxy/extension_proxy.coston2.docker.toml`), then restart the proxy.
