...
- **Private Key Management:** TEE machines securely generate, store, back up, and restore private keys, enabling multi-signature wallet operations across blockchains.
  See [TEE public and private keys](/fcc/tee-keys).
- **Decentralized Consensus:** Instructions are relayed to TEE machines only after reaching a 50%+ signature weight from Flare's data providers, leveraging the same [signing policy](/network/fsp) used across the Flare Systems Protocol.
  See [Data providers and cosigners](/fcc/data-providers).

:::tip[Interested in learning more?]
For a detailed explanation of the FCC mechanism, read the [FCC whitepaper](/pdf/whitepapers/20260706-FlareConfidentialCompute.pdf).
:::

:::warning
Flare Confidential Compute is in the final stages of development.
It is available on **Coston2** and **Coston** today and will be available on other Flare networks soon.
You can already start building Flare Compute Extensions with the [getting started guide](/fcc/guides/getting-started) and the example extension guides under Developer Guides.
:::
...
2. **Data Providers and Cosigners** function as instruction relayers.
   ...
   Details: [Data providers and cosigners](/fcc/data-providers).
...
3. **TEE Machines** verify that instructions carry adequate consensus (threshold signature weight) from data providers and cosigners.
   ...
   Details: [TEE public and private keys](/fcc/tee-keys).
...
Learn how FCC signing works:

- [TEE public and private keys](/fcc/tee-keys) — identity keypair, ECDSA, ECIES, and decrypting inside the enclave.
- [Data providers and cosigners](/fcc/data-providers) — who may send the TEE work, signing weight, and the extra authorization gate.

Then build an extension with [Build Your First Extension](/fcc/guides/getting-started), or the [Private Key Extension](/fcc/guides/sign-extension) example.

Contract ABIs: [FCC Reference](/fcc/reference) — [`IFlareTeeManager`](/fcc/reference/IFlareTeeManager), [`IExtensionManager`](/fcc/reference/IExtensionManager), [`IMachineManager`](/fcc/reference/IMachineManager), [`IVerification`](/fcc/reference/IVerification), [`IOperationFees`](/fcc/reference/IOperationFees), [`IOwnerAllowlist`](/fcc/reference/IOwnerAllowlist), [`ITeeCommonErrors`](/fcc/reference/ITeeCommonErrors).
