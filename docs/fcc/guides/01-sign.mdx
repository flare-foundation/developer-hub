...
:::tip[Quick start]
After completing steps 0–4 (chain selection, deployer keys, contract deploy, ngrok tunnel, and indexer TOML), run:
...
Coston uses different credentials and requires VPN — see `DEPLOYMENT_STEPS.md` in the repository if deploying to Coston instead.
...
The extension ID in `config/extension.env` does not match the on-chain TEE record — usually because `pre-build.sh --force` registered a new extension while an older TEE machine is still registered under the previous extension ID.
