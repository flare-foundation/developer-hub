...
1. A user sends an [Elliptic Curve Integrated Encryption Scheme (ECIES)](https://en.wikipedia.org/wiki/Integrated_Encryption_Scheme) encrypted private key onchain via the `InstructionSender` contract.
...
1. **onchain contract** that sends instructions;
2. **offchain handler** that processes them;
...
- [ngrok](https://ngrok.com/) — to expose your local proxy to the internet.
...
This example is available in three languages, but the Go implementation uses a different architecture than Python and TypeScript.
...
The `handle_key_update` handler decrypts via the TEE node and stores the key; `handle_key_sign` signs the message and ABI-encodes the result — the same logic as the Go implementation, adapted to the Framework API.
...
:::tip[Quick start]
After configuring keys, ngrok, and the indexer TOML (steps 0-4 below):
...
On success, it writes `EXTENSION_ID` and `INSTRUCTION_SENDER` to `config/extension.env` — do not edit this file manually unless recovering from an error.

:::warning
`pre-build.sh` refuses to re-run if `config/extension.env` already exists.
Use `./scripts/pre-build.sh --force` only when you intentionally need a new extension.
Re-running against an existing TEE orphans it and causes `test.sh` to fail with `MachineManager.TooMany()`.
:::

### Step 3: Start the ngrok tunnel

In a separate terminal, expose the proxy's public port:
...
:::info
ngrok's free tier provides one reserved domain, so this URL normally stays stable across restarts.
...
The Coston2 indexer is reachable without VPN.
Coston uses different credentials and requires Abelium VPN — see `DEPLOYMENT_STEPS.md` in the repository if deploying to Coston instead.

:::info[Flare Indexer Access]
To get the indexer credentials, please get in touch with us via [support](https://flare.network/support) or [X](https://x.com/FlareDevs) and share what you are building.
:::
...
`SIMULATED_TEE=true` in your `.env` pairs with `MODE=1` injected by Docker Compose for simulated attestation.
...
`SIMULATED_TEE` and the container `MODE` env disagree.
...
3. Re-run `post-build.sh` and `test.sh`.
