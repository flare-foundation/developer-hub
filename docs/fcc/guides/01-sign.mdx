...
1. A user sends an ECIES-encrypted private key on-chain via the `InstructionSender` contract
...
````mermaid
      graph LR
          subgraph chain["Coston2 C-chain"]
              IS[InstructionSender]
              TER[TeeExtensionRegistry]
              TMR[TeeMachineRegistry]
          end
          ...
          subgraph tunnel["Tunnel (cloudflared / ngrok)"]
              url["Public HTTPS URL"]
          end

          proxy <-- "read instructions /<br/>submit results" --> chain
          url <--> proxy
      ```
...
We will now walk through deploying the extension end-to-end on the Coston2 testnet (chain ID 114).
