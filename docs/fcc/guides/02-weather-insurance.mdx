...
2. `buyPolicyPrivate` sends the ciphertext as a `WEATHER` / `BUY` instruction.
...
`processWeatherFetch` ABI-decodes a city name, calls the OpenWeatherMap API, and returns a JSON `WeatherReport` in `ActionResult.Data`:
...
`processWeatherSettle` fetches daily precipitation from the OpenWeatherMap API and ABI-encodes the settlement:
...
`processWeatherBuy` decrypts ECIES ciphertext via the TEE node's `/decrypt` endpoint, validates `PrivateBuyParams`, stores terms in memory, and returns ABI-encoded parameters for `relayPrivateBuy`:
...
`PAY_TOKEN` is a mocked [ERC-20](https://eips.ethereum.org/EIPS/eip-20) token on Coston2 — used for premiums and payouts during testing.
...
`pre-build.sh` compiles Solidity, deploys `WeatherInsurance`, and registers the extension on-chain.
It writes `EXTENSION_ID` and `INSTRUCTION_SENDER` to `config/extension.env`.

`extension-setup.sh` calls `setPayToken` on the deployed contract.
...
`post-build.sh` allows the TEE version and registers the machine.

`extension-post-setup.sh` reads the TEE signing address from the proxy and calls `setTeeAddress` on `WeatherInsurance`.
