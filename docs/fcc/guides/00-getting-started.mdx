...
Build your first **Flare Confidential Compute (FCC)** extension from the Hello World scaffold example, and use this guide to follow the setup and testing flow.
Use this guide to see how extensions fit into the TEE stack, which files you customize, and how to deploy and test on **Coston2** testnet.
...
First, clone the scaffold and its build dependencies so you can move on to the instruction lifecycle and the files you own.
Understand the instruction lifecycle and the files you own.
Then, deploy the Hello World smart contract and TEE stack against the Coston2 testnet.
Finally, customize the OPType and OPCommand handlers, as well as the on-chain entry point, for your own extension.
...
Docker builds for this scaffold example expect a sibling layout under a common parent directory, so set up the repository structure before you build.
Before you begin, clone the required repositories into that layout so the setup matches the rest of this guide.
The extension module uses a `replace` directive for [`tee-node`](https://github.com/flare-foundation/tee-node), and `start-services.sh` builds [`tee-proxy`](https://github.com/flare-foundation/tee-proxy) unless you set `REGISTRY` to a remote image registry.
...
The scaffold is a complete Hello World example, so keep the HTTP server and deploy scripts while you change the files that keep the OPType and OPCommand strings aligned across Solidity and Go.
To build your own extension, keep the HTTP server and deploy scripts, and change these files so the OPType and the OPCommand strings stay aligned across Solidity and Go:
...
`SAY_HELLO` uses JSON in `OriginalMessage`.
`SAY_GOODBYE` uses ABI encoding so the contract can accept typed `string` arguments and `abi.encode` them for the TEE.
...
`pre-build`, `post-build`, `start-services`, and `test` all use `EXT_PROXY_URL`.
...
This allows your TEE code version for the extension, then registers the TEE machine onchain (using `EXT_PROXY_URL` and `NORMAL_PROXY_URL` from `.env`).
...
| Docker build cannot find `tee-node`              | Confirm the sibling clone layout under `tee/` described in [Clone the Scaffold](#clone-the-scaffold-example).     |
