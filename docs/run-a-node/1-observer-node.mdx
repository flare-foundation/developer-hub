...
1. [On bare-metal](#setup-on-bare-metal): This method is more complex but offers better performance and lower memory usage.

2. [With Docker](#setup-with-docker): This approach is simpler but results in lower performance and higher memory usage.
...
<Tabs groupId="network" block>
  ...
  <TabItem value="coston" label="Songbird Testnet Coston">
    ...
    | **[GCC](https://gcc.gnu.org/install/)**           | latest         |
    | **[jq](https://stedolan.github.io/jq/download/)** | latest         |
    | **[Ubuntu](https://ubuntu.com)**                  | 20.04 or 22.04 |
...
<Tabs groupId="network" block>
  ...
  <TabItem value="songbird" label="Songbird Canary-Network">
    ...
    The resulting executable will be stored in `build/flare`.

  </TabItem>
  <TabItem value="coston" label="Songbird Testnet Coston">

    ```bash
    git clone --branch v0.6.6-coston https://github.com/flare-foundation/go-flare.git
    cd go-flare/avalanchego
    chmod +x scripts/build.sh
    ./scripts/build.sh
    ```
...
<Tabs groupId="network" block>
  ...
  <TabItem value="songbird" label="Songbird Canary-Network">
    ...
    While the Songbird Canary-Network is being tested, all nodes wanting to peer with it, including observer nodes, need to have their IP address whitelisted.
...
- [`--chain-config-dir`](https://docs.avax.network/nodes/maintain/avalanchego-config-flags#--chain-config-dir-string): Optional JSON configuration file for using non-default values.
  ...
  <details>
  <summary>Sample JSON configuration for observer nodes.</summary>
...
<Tabs groupId="network" block>
  <TabItem value="flare" label="Flare Mainnet" default>
    ...
    Use the Docker image at [go-flare](https://hub.docker.com/r/flarefoundation/go-flare). The **Overview** tab in the repository linked explains the configurable parameters.

    Download and start the container:

    ```bash
    docker pull flarefoundation/go-flare
  ...
  <TabItem value="songbird" label="Songbird Canary-Network">
    ...
    Use the Docker image at [go-songbird](https://hub.docker.com/r/flarefoundation/go-songbird). The **Overview** tab in the repository linked explains the configurable parameters.

    Download and start the container:

    ```bash
    docker pull flarefoundation/go-songbird
  ...
  <TabItem value="coston" label="Songbird Testnet Coston">
    ...
    Use the Docker image at [go-flare](https://hub.docker.com/layers/flarefoundation/go-flare/v0.6.6-coston/images/sha256-b88cf18baf5300a4e15b630dd6de669e48ab316a22cffd370961cdd68226ef0a?context=explore). The **Overview** tab in the repository linked explains the configurable parameters.

    Download and start the container:

    ```bash
    docker pull flarefoundation/go-flare:v0.6.6-coston
  ...
  <TabItem value="coston" label="Songbird Testnet Coston">
    ...
    services:
      observer:
        ...
        image: flarefoundation/go-flare:v0.6.6-coston
        restart: on-failure
        environment:
          - AUTOCONFIGURE_BOOTSTRAP=1
          - NETWORK_ID=coston
