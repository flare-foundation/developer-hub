...
An observer node does not participate in network consensus. Unlike [validator nodes](validator-node), which provide state consensus and add blocks, observer nodes remain outside the network and have no effect on consensus or blocks.
...
- Transactions are sent directly to the network instead of through a third party.
- Public nodes are usually rate-limited, your own node does not have such restriction.
...
For archival nodes, which keep the whole history of the blockchain, pruning should be disabled.
:::
...
- CPU: 4 cores, or more
- RAM: 16 GB, or more
- Disk: 1 TB SSD, or better (For archival nodes 4 TB SSD, or better)

### Docker

Docker Hub contains container images for all `go-flare` releases at [flare-foundation/go-flare](https://hub.docker.com/r/flarefoundation/go-flare).
