...
A new voting epoch (or reward epoch, respectively) starts immediately after the previous one concludes.
...
- **Capped Delegation Weight ($W'_{WFLR}$)**: The delegation weight, $W_{WFLR}$, limited to 2.5% of total `WFLR` supply (read from the `WNat` contract).
