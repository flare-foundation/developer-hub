...
1. **Protocol Manager Service**: Handles periodic transactions for each voting round by querying protocol data providers.
...
4. **Voter Registration Service**: Registers data providers on the [`VoterRegistry`](/network/fsp/solidity-reference/IVoterRegistry) contract.
...
- Registers the data provider on the `VoterRegistry` contract before the `SigningPolicyInitialized` event signals the end of the registration period.
