...
Flare's sub-protocols utilize a uniform signing weight for decision-making; however, reward distribution is based on the delegators' participation weights.
Each sub-protocol calculates rewards for various participation weight types, allowing delegators to claim rewards according to their specific contribution shares.
...
Sub-protocols implement their own contracts to gather rewarding inputs, which are passed to the Reward Manager for distribution.
Funding is provided by sub-protocol-specific contracts, which may also handle inflation-based rewards through automated offers.
...
- Newly registered data providers start with zero passes.
  If a provider held passes but missed the registration for an epoch, their pass count is also reset to zero for the next registered epoch.
...
2. **Finalization Rewards**:
   ...
   - During a 20s grace period, a set of selected data providers gets rewarded either for successful finalization or an attempt of it.
     (i.e. If the call got reverted due to data being already finalized.)
...
<div style={{ textAlign: "center", margin: "30px 0" }}>
  <ThemedImage
    ...
    sources={{
      light: useBaseUrl("img/docs/network/fsp/finalization.light.svg"),
      dark: useBaseUrl("img/docs/network/fsp/finalization.dark.svg"),
    }}
  />
</div>
...
- Each selected provider that finalizes within the grace period receives an **equal share** of the finalization reward (i.e. `V(i)/N(i)`, where `N(i)` is the number of selected providers), regardless of their individual weights; that share is then split among the provider and its delegators as usual.
