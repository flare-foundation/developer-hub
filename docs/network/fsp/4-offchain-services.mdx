...
- `submit1`, `submit2`: Data submission at fixed scheduled times during the voting epoch (`submit3` exists but is currently reserved for future use).
...
- `message` (38 bytes):
  ...
  - `secureRandom` (1 byte): boolean flag (`1` = true, `0` = false) indicating whether the round's random number is secure
  - `merkleRoot` (32 bytes)
