...
Data providers are off-chain participants in Flare's protocols, responsible for threshold-weighted voting across all sub-protocols.
...
The **Signing Policy Definition Protocol** sets the eligible data providers, their weights, and the threshold required for confirmation before each reward epoch begins.
...
|                | • Scaling: Signing and Finalization<br />• Block-latency: Sortition     | $W_S$            |
