...
- `claimType`: The category of the reward claim, which can be one of the following:
  ...
  - `fee`: Similar to 'direct' but specific to eligible data providers. Covers delegation fees (WFLR) and node staking fees.
  - `wflr`: Rewards for eligible data providers, distributed to delegators based on their participation in WFLR.
  - `mirror`: Rewards distributed to delegators based on their participation in WM.
  - `cchain`: Rewards for eligible data providers, distributed based on their share in WC.

Weight-based claims (`wflr`, `mirror`, `cchain`) represent total rewards for data providers, to be distributed according to the delegators' participation in the data providers' weight.
...
1. **Finalization Rewards**:
   ...
   - To avoid centralization, rewards prioritize eligible data providers and distribute based on their weight.
...
2. **Signing Deposition Rewards**:
   - Data providers submitting signatures within a grace period (10s) receive rewards.
...
- A hash value is used to determine which providers are eligible for finalization rewards based on their signing weight.
- Providers are selected in proportion to their weights, and rewards are distributed among them.
...
2. **Signing Deposition**: Rewards for providers who timely submit valid signatures.
