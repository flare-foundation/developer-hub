...
- **Process**:
  ...
  - **Signing Policy Snapshot**: The new signing policy is finalized and stored on-chain, marking the start of the new reward epoch.
- **Incentivization**:
...
- **Data Collection**: Voters collect stake and delegation data from the P-chain that started in a voting epoch.
- **Merkle Tree Construction**: Organize the data into a Merkle tree for efficient storage and verification.
- **Signing and Finalization**: Voters sign the Merkle root, and upon reaching the threshold, the root is finalized on the Relay Contract.
...
- **Consensus Mechanism**:
  - Voters sign a hash of the aggregated list of node IDs.
  - Confirmation requires signatures exceeding the threshold weight.
...
- **Data Collection**:
  ...
  - Voters collect partial reward claims from each sub-protocol.
...
- **submitAndPass**: A general-purpose function that proxies calls to external functions.
...
- **Reward Distribution**: Facilitates the claiming and distribution of rewards to voters, delegators, and stakers.
