...
- `MaxBuildWindows`: 60 (5 minutes) — how far ahead a node schedules its own build attempt.
  Note that a node searches up to `MaxLookAheadSlots` (720 slots) for its next slot.
