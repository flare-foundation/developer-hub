...
Wrapped tokens are stored on the blockchain in a [`ERC-20`](https://ethereum.org/en/developers/docs/standards/tokens/erc-20/) token standard in a smart contract called `WNat`.

The [`IWNat`](/network/solidity-reference/IWNat) contract is the interface for the Wrapped Native Token contract.

In this guide, we'll use the `deposit` and `withdraw` functions to wrap and unwrap native tokens:

## Deposit

The `deposit` function wraps the amount of native tokens you specify in the field.

You can deposit native tokens on the Flare network using the `deposit` function using the following code:

<CodeBlock
  language="typescript"
  title="scripts/wnat/deposit.ts"
>
  {Deposit}
</CodeBlock>
...
<CodeBlock
  language="typescript"
  title="scripts/wnat/withdraw.ts"
>
  {Deposit}
</CodeBlock>
<br />
