...
This guide is for developers who want to interact with Flare using JavaScript.
In this guide, using JavaScript, you will learn how to:
...
Copy the `FtsoV2FeedConsumer` sample contract code given below, and save the `.sol` file in the same folder as your `package.json`.
...
This will generate two files `build/FtsoV2FeedConsumer.abi` and `build/FtsoV2FeedConsumer.bin` files with the contract's ABI and bytecode.
Rename `FtsoV2FeedConsumer.abi` to `FtsoV2FeedConsumer.json`.
...
In a `deployContract.js` file, you can define the following code to deploy the contract:
...
You can now run the `deployContract.js` script to deploy the contract.
...
Congratulations!
You have now successfully deployed a contract on Flare using JavaScript.
