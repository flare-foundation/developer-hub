...
This pattern separates **authorization** (user intent) from **execution** (onchain relay).

EIP-3009 adds two core functions, both of which include timestamps to prevent stale authorizations, and a **nonce** to prevent replay attacks:
...
- USD₮0 contract addresses and ABI (see `TetherTokenOFTExtension` in [USD₮0 docs](https://docs.usdt0.to/technical-documentation/developer#flare)).
  <details>
