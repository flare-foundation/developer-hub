...
An observer node is a node that observes the network and submits transactions. Unlike [validator nodes](validator-node-client), which provide state consensus and add blocks, observer nodes remain outside the network and have no effect on consensus or blocks.
...
- CPU: 8 cores, or more
- RAM: 16 GB, or more
- Disk: 500 GB NVME SSD, or better
...
Node syncing can be stopped at any time using `Ctrl-C`. Use the same command as before to restart the node syncing. Synchronization will resume where it left if it is interrupted.
...
- [`--chain-config-dir`](https://docs.avax.network/nodes/maintain/avalanchego-config-flags#--chain-config-dir-string):
    ...
    :::note Sample configuration file for observer nodes
...
An archival node keeps the whole history of the blockchain, instead of [pruning](glossary.md#pruning) old transactions which is the default setting.
