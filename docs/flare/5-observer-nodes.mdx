...
An observer node is a node that observes the network and submits transactions. Unlike [validator nodes](validator-nodes), which provide state consensus and add blocks, observer nodes remain outside the network and have no effect on consensus or blocks.
...
An archival node keeps the whole history of the blockchain, instead of pruning old transactions which is the default setting.
