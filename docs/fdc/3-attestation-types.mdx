...
- `AddressValidity`: Determines whether a given string represents a valid address on an external blockchain.

- `BalanceDecreasingTransaction`: Detects a transaction that either decreases the balance of a specified address.

- `ConfirmedBlockHeightExists`: Verifies whether a block with the specified blockNumber is confirmed.

- `EVMTransaction`: Fetches information about an EVM transaction, including details on associated events if specified.

- `Payment`: Relays information about a transaction on an external chain that is classified as a native currency payment.

- `ReferencedPaymentNonexistence`: Asserts that a payment agreed to be completed by a certain deadline, has not been made.
...
- [Bitcoin (`Base58`)](/fdc/chain-configuration#bitcoin-base58)
- [Bitcoin (`Bech32`)](/fdc/chain-configuration#bitcoin-bech32)
- [Dogecoin (`Base58`)](/fdc/chain-configuration#dogecoin-base58)
- [XRPL (`Base58`)](/fdc/chain-configuration#xrpl-base58)
...
2. If the transaction cannot be fetched or is in a block with insufficient [confirmations](/fdc/chain-configuration#finality), the attestation request is rejected.
...
- `sourceAddressIndicator`: The [standard address hash](/fdc/chain-configuration#standard-address-hash) of the address in question.
...
3. The required confirmations are defined based on chain-specific [finality](/fdc/chain-configuration#finality).
...
| `standardPaymentReference`     | `bytes32`     | [Standard payment reference](/fdc/chain-configuration#standard-payment-reference).                                             |
| `oneToOne`                     | `bool`        | Indicates if the transaction involves only one source and one receiver.                                                        |
| `status`                       | `uint8`       | [Transaction success status](/fdc/chain-configuration#transaction-success-status).                                             |

### Verification Process

1. The transaction identified by `transactionId` is fetched from the relevant blockchain node or indexer.
2. If the transaction cannot be retrieved or is in a block with insufficient [confirmations](/fdc/chain-configuration#finality), the attestation request is rejected.
3. Once the transaction data is fetched, a [payment summary](/fdc/chain-configuration#payment-summary) is computed according to the chain-specific rules.
...
1. **Block Confirmation**:
   ...
   - If the `firstOverflowBlock` cannot be determined or lacks the required [number of confirmations](/fdc/chain-configuration#finality), the request is rejected.
...
- **Transaction Criteria**:
  ...
  - The transaction must include the specified [standard payment reference](/fdc/chain-configuration#standard-payment-reference).
