...
2. **Check the voting round on the Systems Explorer:**
   Visit `https://coston2-systems-explorer.flare.network/voting-round/{roundId}?tab=fdc` to see if any attestations were included.
...
`Web2Json` currently has no host whitelist on Coston or Coston2.
Use the `PublicWeb2` source; any public HTTPS JSON endpoint can be requested.

Flare Mainnet and Songbird have no Web2Json source.
[`requestAttestation`](/fdc/reference/IFdcHub#requestattestation) reverts for `Web2Json` on those networks.
