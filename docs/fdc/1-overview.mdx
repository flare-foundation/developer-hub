...
- **Current State**: A proof can only be constructed for sufficiently recent states (transactions, API responses, ...).
  The maximum allowed age of the underlying data is set per attestation type and data source; for most chain-data types it is 14 days, while some types (such as `AddressValidity` and `Web2Json`) place no practical limit.
...
- **Web2Json**: Fetches and processes Web2 JSON using a JQ transformation, then returns it as ABI-encoded output.
  Available on **Coston and Coston2** with the `PublicWeb2` source.
  Not available on Flare Mainnet or Songbird.
...
AddressValidity, EVMTransaction, and Payment are the attestation types most used in production applications.
The last three types are primarily used in **FAssets**.
Web2Json is available on Coston and Coston2 only.
...
3. **Submit Request**: Use [`requestAttestation`](/fdc/reference/IFdcHub#requestattestation) from `FdcHub` and pay the required fee.
   The fee must be at least the minimum configured for the attestation type and source; otherwise the request is rejected.
   Larger fees increase the chance that a request is confirmed in its round.
   The fee of a confirmed request is awarded to the data providers, while the fee of an unconfirmed request is burnt.
...
5. **Achieve Consensus**: Aggregate BitVectors to form a consensus BitVector that is supported by more than 50% of the total weight of data providers.
