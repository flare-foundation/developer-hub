...
To properly decode data from the [JsonApi](/fdc/attestation-types/json-api) attestation, we need to define a data structure that matches our expected format. Following patterns from the [JsonApi attestation type guide](/fdc/guides/json-api), we create a simple `DataTransportObject` struct:
...
This structure contains a single field to store the reserve amount received from the Web2 API. With this definition in place, we can now decode the `abi_encoded_data` within the `IJsonApi.Proof` struct and access its `reserves` field.
Before accessing this data, we must first validate the proof using the Flare verification system.
...
This guide demonstrates deployment on Flare's Coston and Coston2 testnets, but the same approach can be adapted for any compatible EVM chain.
The complete implementation process follows these sequential steps:
...
The filter removes all commas (separating thousands), and removes the decimal part.
