...
```bash
forge script script/fdcExample/Web2Json.s.sol:PrepareAttestationRequest --private-key $PRIVATE_KEY --rpc-url $COSTON2_RPC_URL --etherscan-api-key $FLARE_RPC_API_KEY --broadcast  --ffi
```
...
You can check if the request was submitted successfully on the [AttestationRequests](https://coston2-systems-explorer.flare.network/attestation-request) page on the Flare Systems Explorer website.
...
```solidity title="script/fdcExample/DeployContract.s.sol"
