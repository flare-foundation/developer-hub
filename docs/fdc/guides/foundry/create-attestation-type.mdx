...
This guide is for developers who want to extend FDC by creating a new attestation type.
...
2. **Create a definition file:** In the root of the cloned repo a template definition file (`ITypeTemplate.sol.example`) is provided.
   Rename the file to `ICustomType.sol` and the interface defined within the file to to `ICustomType`.
   You can use any name in place of `CustomType` upto 32 ASCII characters.
...
3. **Modify `@custom` props:**
   ...
   - `@custom:supported`: Indicate the data source, currently supported sources - `BTC`, `DOGE` ,`XRP`, `FLR`, `SGB`, `ETH` and `WEB2`.
     A single type can support multiple data sources.
...
4. **Define request and response structs:** Define the inputs to the `RequestBody` and `ResponseBody` structs.
