...
contract MyStablecoin is ERC20, ERC20Burnable, Ownable, ERC20Permit {
    ...
    ) ERC20("MyStablecoin", "MST") Ownable(initialOwner) ERC20Permit("MyStablecoin") {
...
contract ProofOfReserves is Ownable {
    ...
    uint256 public debugTokenReserves;
    uint256 public debugClaimedReserves;
    ...
    function verifyReserves(
        ...
        for (uint256 i = 0; i < transactionProofs.length; i++) {
            totalTokenSupply += readReserves(transactionProofs[i]);
        }
        debugTokenReserves = totalTokenSupply;
...
:::note
The intermediate `.txt` filenames shown below (`_token<chainId>.txt`, `_proofOfReserves<chainId>.txt`, `_EVMTransaction_Coston_request.txt`, `_Web2Json_roundId.txt`, etc.) describe the **intended**, consistent naming.
The current `flare-foundry-starter` is being aligned to this scheme in a follow-up PR — until then, the files actually written and read by the script may differ in their leading-underscore and separator placement.
If you run the starter and a step can't find the expected file, double-check the filename hyphenation in the failing script.
:::
