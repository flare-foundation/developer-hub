...
In this guide, we will be checking wether the string `rPT1Sjq2YGrBMTttX4GZHjKu9dyfzbpAYe` is a valid XRPL testnet address.
This is an example of the `AddressValidity` attestation request.
We chose it for demonstration purposes because requires a single input parameter, but the same process would work for any of the FDC [attestation types](https://dev.flare.network/fdc/attestation-types).
...
Then click `Execute`.
The `abiEncodedRequest` will appear under the `Response body` section bellow, along with the request status which should be `VALID`.
...
There are two additional values withing the request body that we have no yet properly explained.
...
The full list of values is available bellow.
...
It is one of the official Flare contracts, and it is deploy on each of Flare chains.
...
The request fee can be querried from the `FdcRequestFeeConfigurations` contract, through its `getRequestFee` function ([Flare Explorer](https://coston2-explorer.flare.network/address/0x191a1282Ac700edE65c5B0AaF313BAcC3eA7fC7e?tab=read_contract), [Flarescan](https://coston2.testnet.flarescan.com/address/0x191a1282Ac700edE65c5B0AaF313BAcC3eA7fC7e/contract/114/readContract)).
...
We input the `abiEncodedRequest` value, and the ammount returned by the `getRequestFee` function.
...
- Flare  
   - [Flare Explorer](https://flare-explorer.flare.network/address/0x259852Ae6d5085bDc0650D3887825f7b76F0c4fe) - [Flarescan](https://flare.flarescan.com/address/0x259852Ae6d5085bDc0650D3887825f7b76F0c4fe)
  </details>
...
To check wether the round has finalized, we need to know the block in which the attestation request transaction was made;
...
Once we know the voting round ID, we can to the `Finalizations` tab of the [Coston2 Systems Explorer](https://coston2-systems-explorer.flare.rocks/finalizations).
...
Once the round has finalized, we can prepare the proof request using the [Flare Data Availability Client](https://ctn2-data-availability.flare.network/api-doc#/) interactive documentation.
...
The Data Availability Client informs us that the address is, indded, a valid XRPL address.
...
The `curl` command is displey as the first part of the `Responses` section.
In this case, it is:
