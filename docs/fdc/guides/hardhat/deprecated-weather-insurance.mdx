---
title: Weather Insurance (deprecated)
...
You can find the updated version of this guide [here](/fdc/guides/hardhat/weather-insurance).
:::

In this guide, we will examine an example of a simple insurance dApp that uses the FDC's [JsonApi](/fdc/guides/hardhat/deprecated-json-api) attestation type.
...
We will use the [Flare Data Connector](/fdc/overview) to collect the weather data so, in keeping with the [JsonApi guide](/fdc/guides/hardhat/deprecated-json-api), we start by defining a `DataTransportObject` which will determine how the FDC should encode the data.
...
The `IJsonAPi.Proof` is validated as demonstrated in the [JsonApi guide](/fdc/guides/hardhat/deprecated-json-api).
...
Like in the [JsonApi guide](/fdc/guides/hardhat/deprecated-json-api), we define an `abiSignatureHack` function, that will allow us to extract the ABI signature of the `DataTransportObject` from the Hardhat-generated artifacts.
...
To learn more about the JsonApi attestation request look at the [related guide](/fdc/guides/hardhat/deprecated-json-api), or its [spec](/fdc/attestation-types/deprecated-json-api).
...
The intermediate steps more or less follow the ones in the [JsonApi guide](/fdc/guides/hardhat/deprecated-json-api).
