...
From that and the array of proofs, it constructs an `IPayment.Proof` object, on which it call the `registerAddress` function of the `AddressRegistry` contract deployed above.
