---
title: Proof of Reserves (deprecated)
...
You can find the updated version of this guide [here](/fdc/guides/hardhat/proof-of-reserves).
:::

This is a guide on how to build a simple dApp using the [Flare Data Connector](/fdc/overview).
It demonstrates how multiple attestation types, namely the [EVMTransaction](/fdc/attestation-types/evm-transaction) and [JsonApi](/fdc/attestation-types/deprecated-json-api), can be combined within the same app.
...
The [Flare Data Connector (FDC)](/fdc/overview) provides solutions for both accessing Web2 APIs through the [JsonApi](/fdc/attestation-types/deprecated-json-api) attestation type and collecting cross-chain data via the [EVMTransaction](/fdc/attestation-types/evm-transaction) attestation type.
...
To properly decode data from the [JsonApi](/fdc/attestation-types/deprecated-json-api) attestation, we need to define a data structure that matches our expected format.
Following patterns from the [JsonApi attestation type guide](/fdc/guides/foundry/deprecated-json-api), we create a simple `DataTransportObject` struct:
...
For a more detailed explanation of the included fields, look at the appropriate type specification ([IEVMTransaction](/fdc/attestation-types/evm-transaction) and [JsonApi](/fdc/attestation-types/deprecated-json-api)).
