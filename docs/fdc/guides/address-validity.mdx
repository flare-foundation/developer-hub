...
The AddressValidity attestation type validates whether a string represents a valid address on supported blockchain networks (`BTC`, `DOGE`, and `XRP`).
...
In this guide, we will follow the steps outlined in the [FDC overview](/fdc/guides/fdc-overview).
...
- `sourceId`: UTF8 hex string encoding of `testBTC`, zero-padded to 32 bytes.
  ...
  Since this is a demo, we will only deploy our contract on one of the Flare testchains (either Coston or Coston2); these only allow for lookup on the corresponding testchains.
...
In the [example repository](https://github.com/flare-foundation/flare-foundry-starter), these are shipped within the [Base](https://github.com/flare-foundation/flare-foundry-starter/blob/master/script/fdcExample/Base.s.sol) library, since all attestation type examples make use of them.
...
:::note
This step marks our transition from off-chain preparation to on-chain interaction with the FDC protocol.
We will now submit our validated request to the blockchain using deployed smart contracts.
:::
