...
The manager is then notified via `handleMintedFAssets` so it can route FXRP to the appropriate [smart account](/smart-accounts/overview) based on the XRPL `sourceAddress` and `memoData`.
...
See [Minting Troubleshooting — smart account path](/fassets/troubleshooting/minting-troubleshooting#smart-account-path) and [unrecognized memo routes to smart accounts](/fassets/troubleshooting/minting-troubleshooting#4-unrecognized-memo-routes-to-smart-accounts).
...
See [Minting Troubleshooting — payment below minimum fee](/fassets/troubleshooting/minting-troubleshooting#1-payment-below-the-minimum-minting-fee) for prevention guidance.
...
Emitted when a direct minting is throttled by the **hourly or daily** [rate limiter](/fassets/minting#rate-limits) and its execution is postponed.

For amounts at or above [`getDirectMintingLargeMintingThresholdUBA()`](/fassets/reference/IAssetManager#getdirectmintinglargemintingthresholduba), see [`LargeDirectMintingDelayed`](#largedirectmintingdelayed) instead.
...
Emitted when [`executeDirectMinting`](/fassets/reference/IAssetManager#executedirectminting) is called for a payment whose underlying amount is **greater than or equal to** [`getDirectMintingLargeMintingThresholdUBA()`](/fassets/reference/IAssetManager#getdirectmintinglargemintingthresholduba).
...
See [Large minting delay](/fassets/minting#large-minting-delay) and [Minting Troubleshooting — delays](/fassets/troubleshooting/minting-troubleshooting#delays).
...
See [Minting Troubleshooting — delays](/fassets/troubleshooting/minting-troubleshooting#delays).
...
- `valueUBA`: Redeemed amount after the [system fee](/fassets/redemption#system-fee).
  The agent pays `valueUBA` minus `feeUBA`.
- `feeUBA`: The [agent fee](/fassets/redemption#agent-fee) (`underlyingFeeUBA`), taken from `valueUBA`.
...
Emitted when a redemption request is created and the system fee is minted to `systemRedemptionFeeReceiver`.
The redeemed amount on the request is lowered by this fee.
See [System fee](/fassets/redemption#system-fee).

Parameters:

- `agentVault`: Address of the agent vault for this redemption request.
- `requestId`: Unique identifier for the redemption request.
- `feeUBA`: System fee minted to the receiver, in underlying base units.
...
Emitted when the agent's share of the [agent fee](/fassets/redemption#agent-fee) is minted as FAssets and paid to that agent's collateral pool.
This happens when the underlying payment is confirmed, after the system fee was already taken.
