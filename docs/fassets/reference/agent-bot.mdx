...
Cancel a pending transfer of underlying assets to the core vault by specifying the agent vault address.

```bash
yarn agent-bot cancelTransferToCoreVault <agentVaultAddress> [options]
```

Options:

- `--force`: Force cancel transfer even if conditions are not met
