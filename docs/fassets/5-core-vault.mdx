...
The **Core Vault (CV)** is the backbone of the FAssets system's security model.  
It solves a critical challenge in cross-chain systems: **ensuring that agents cannot steal user funds while still allowing the system to scale.**
...
- **Stops theft at the root** by removing agent control over pooled funds.
