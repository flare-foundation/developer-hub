...
{/* prettier-ignore */}
<RemixEmbed fileName="FAssetsRedeem.sol">Open in Remix</RemixEmbed>
...
The `RedemptionRequested` event contains the agent vault address, redeemer address, underlying chain address, the amount left after the system fee (`valueUBA`), the agent fee (`feeUBA`), and other important information.
The system fee is a separate mint, recorded by [`SystemRedemptionFeePaid`](/fassets/reference/IAssetManagerEvents#systemredemptionfeepaid).
See [Fees](/fassets/redemption#fees).
...
- The amount after the system fee (`valueUBA`) is `10000000`;
- The agent fee (`feeUBA`) is `50000`;
...
- Learn how to [mint FAssets](/fassets/developer-guides/fassets-mint).
