...
This process is called **self-minting**.
When an agent self-mints:

- The agent pays the amount to mint on the underlying chain and executes the minting.
- The self-minting operation adds a [ticket to the redemption queue](/fassets/redemption#redemption-tickets-and-the-redemption-queue), alongside tickets from other users.
- Agents can create vaults that are not public, so the vault can only be used to self-mint.
...
For developer resources, see the [fassets-demo-dapp](https://github.com/flare-foundation/fassets-demo-dapp) source ([deployed instance](https://fassets-demo-dapp.vercel.app/)), our [developer guides](/fassets/developer-guides), or start with the [FXRP address](/fxrp/token-interactions/fxrp-address), [minting](/fassets/developer-guides/fassets-mint), and [redemption](/fassets/developer-guides/fassets-redeem) guides.
