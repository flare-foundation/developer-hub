...
3. Each chosen ticket belongs to an agent.
   ...
   - Amount the agent must pay on the underlying chain, after the [system fee and the agent fee](#fees).
...
A redemption request charges two fees, one after the other, plus an optional executor fee.
Both rates are in BIPS (10,000 BIPS = 100%) and each one is rounded down.
Live rates are in the [operational parameters](/fassets/operational-parameters).

| Fee              | Charged in                                                           | When it applies                                                        | Who receives it                                                                |
| :--------------- | :------------------------------------------------------------------- | :--------------------------------------------------------------------- | :----------------------------------------------------------------------------- |
| **System fee**   | FAssets minted to the fee receiver; taken out of the redeemed amount | A receiver is set, and the request is not a transfer to the core vault | [`systemRedemptionFeeReceiver`](#system-fee), the address set by governance    |
| **Agent fee**    | Underlying asset, deducted from the payout                           | Every redemption request except a transfer to the core vault           | The agent. A share is then minted as FAssets into that agent's collateral pool |
| **Executor fee** | FLR                                                                  | Only when the redeemer names an executor                               | The executor, for calling redemption default if the agent does not pay         |

### Redemption fee

On a redemption of amount \(x\), the redeemer is paid this much underlying:

$$
x \cdot \left(1 - \frac{\text{systemRedemptionFeeBIPS}}{10000}\right) \cdot \left(1 - \frac{\text{redemptionFeeBIPS}}{10000}\right)
$$

The redeemer calls `redeem`, `redeemAmount`, or `redeemWithTag`.
Inside that call the Asset Manager creates one request per agent and applies the fees below.
A transfer to the core vault creates a request too, and skips both fees.

#### System fee

Taken first, in `RedemptionRequests.createRedemptionRequest`.

`systemRedemptionFeeBIPS` is subtracted from the redeemed amount.
The backing for that slice is released, and the same amount of FAssets is minted to `systemRedemptionFeeReceiver`.
[`SystemRedemptionFeePaid`](/fassets/reference/IAssetManagerEvents#systemredemptionfeepaid) records the mint.
The redeemed FAssets, including this slice, are burned from the redeemer, so the receiver's new tokens are the system fee.

Governance sets the rate and the receiver on `RedeemExtendedSettingsFacet` (`setSystemRedemptionFeeBIPS`, `setSystemRedemptionFeeReceiver`).
Read the rate with [`systemRedemptionFeeBIPS`](/fassets/reference/IAssetManager#systemredemptionfeebips) and the receiver with [`systemRedemptionFeeReceiver`](/fassets/reference/IAssetManager#systemredemptionfeereceiver).
The charged rate is the stored rate when the receiver is nonzero, and 0 when the receiver is the zero address.
[`effectiveSystemRedemptionFeeBIPS`](/fassets/reference/IAssetManager#effectivesystemredemptionfeebips) returns that charged rate on networks where the getter is deployed.
The fee is also skipped on a transfer to the core vault.

#### Agent fee

Taken next, on what remains after the system fee.
The global [`redemptionFeeBIPS`](/fassets/operational-parameters) is the same for every agent and is stored on the request as `underlyingFeeUBA`.
The agent pays `underlyingValueUBA - underlyingFeeUBA` on the underlying chain and keeps the difference.
On `RedemptionRequested`, `valueUBA` is the amount after the system fee, and `feeUBA` is this agent fee.

That agent fee is split again when the payment is confirmed, in `RedemptionConfirmationsFacet._mintPoolFee`:

- The agent's own `redemptionPoolFeeShareBIPS` of the fee is minted as new FAssets into that agent's collateral pool (`fAssetFeeDeposited`).
  [`RedemptionPoolFeeMinted`](/fassets/reference/IAssetManagerEvents#redemptionpoolfeeminted) records it.
- The rest stays with the agent as underlying that was never paid out.

The pool share is copied from the agent onto the request when the request is created.
It is charged when the agent (or, after the confirmation window, anyone) proves the underlying payment.
Each agent sets its own share.
The [default agent settings](/fassets/operational-parameters) list the starting value.
...
:::info
    ...
    * Proof of blocked payment: Even if the address is valid, the payment can still be blocked for various reasons.
        ...
        One important example is an XRPL redeemer who has enabled `asfRequireDest` on their account and then used a simple redeem request (`redeem` or `redeemAmount`) instead of [`redeemWithTag`](#redeem-with-tag).
        Incoming payments then require a destination tag, but a simple redeem request does not include one, so the XRPL rejects the agent's payment.
