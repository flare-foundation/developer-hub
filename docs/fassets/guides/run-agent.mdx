...
This suffix will be used for the [FAsset Collateral Pool Token](../overview#pool-collateral). For example, for `FTestXRP`, if you use `MY-ALPHA-AGENT-1`, it would be `FCPT-TXRP-MY-ALPHA-AGENT-1`.
...
To deposit both vault and pool collateral together and let the tool calculate the [minimum required collateral](../overview#the-collateral-ratio) to back the lots, you can use the `depositCollateral` function to the agent, specifying your created agent address in the `AGENT_ADDRESS` and lot size in the `LOTS`, as well exchange `FASSET` with `FTestXRP`, `FTestBTC` or `FTestDOGE` according to which underlying network you are creating the agent:
...
If you don't have available lots, check if the vault and pool [collaterals are enough](../overview#the-collateral-ratio).
...
7. Choose a suffix for your agent's collateral pool and fill in the `poolTokenSuffix` field in the `tmp.agent-settings.json` file with it.
    The `poolTokenSuffix` should only include uppercase letters, numbers, and the `-` symbol.
    This suffix will be used for the [FAsset Collateral Pool Token](../overview#pool-collateral). For example, if you use `MY-ALPHA-AGENT-1`, it would be `FCPT-TBTC-MY-ALPHA-AGENT-1`.
...
7. Choose a suffix for your agent's collateral pool and fill in the `poolTokenSuffix` field in the `tmp.agent-settings.json` file with it.
    The `poolTokenSuffix` should only include uppercase letters, numbers, and the `-` symbol.
    This suffix will be used for the [FAsset Collateral Pool Token](../overview#pool-collateral). For example, if you use `MY-ALPHA-AGENT-1`, it would be `FCPT-TDOGE-MY-ALPHA-AGENT-1`.
