...
1. **Agent Transfers Assets**
    ...
    The agent sends the underlying asset (e.g., XRP) to the Core Vault (CV) address with a valid payment reference.
...
3. **Verification and Redemption**
    ...
    After verifying the payment, the system releases the agent’s collateral, and the agent creates a redemption request for withdrawal.
