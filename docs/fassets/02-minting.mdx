...
keywords: [fassets, xrp, flare-network, minting]
---
...
For XRP, users mint by creating a single payment on the XRP Ledger to the [Core Vault](/fassets/core-vault), then an executor finalizes the mint on Flare.
...
See [Finalizing on Flare](#finalizing-on-flare) for when to use each entry point.
...
- **Minting fee**: A percentage of the received amount (in BIPS), with a minimum floor.
  ...
  See [Minting Troubleshooting](/fassets/troubleshooting/minting-troubleshooting#irreversible-failures) for prevention steps.
...
The standard finalization path for minting.
It mints FAssets to the recipient encoded in the XRPL destination tag or memo.
...
Large mintings are delayed independently by a fixed duration, on a separate branch that skips the hourly and daily windows entirely.

A minting can still be rejected outright, but by a different mechanism: the FAsset-wide minting cap (`mintingCapAMG`) is checked and reserved **before** the rate limits and reverts with `MintingCapExceeded`.

### Large minting delay

A mint is **large** when its underlying amount is **greater than or equal to** [`directMintingLargeMintingThresholdUBA`](/fassets/reference/IAssetManager#getdirectmintinglargemintingthresholduba) — minting exactly at the threshold is already large.
...
Large-mint delay is **exclusive** of the hourly and daily windows: the contract checks the large-mint rule first and, when it applies, never runs the window limiters — so a large minting is never additionally delayed by a window, and it consumes no window capacity.
...
For a mint below the threshold, only the hourly and daily windows apply, and the contract uses whichever of the two pushes `executionAllowedAt` further into the future.
See the [Check Minting Limits guide](/fassets/developer-guides/fassets-mint-limits) for off-chain pre-flight logic.
...
After manual review, governance can release a backlog with [`unblockDirectMintingsUntil`](/fassets/reference/IAssetManager#getdirectmintingsunblockuntiltimestamp).
The call only accepts a timestamp **in the past**: every minting that was already delayed and started before that timestamp becomes executable immediately, including large-minting delays.
It is retroactive only — new mintings are still rate-limited as usual.
...
For delay retry steps and cap failures, see [Minting Troubleshooting — Delays](/fassets/troubleshooting/minting-troubleshooting#delays).
:::
