...
- **FSP (FTSO, FDC)** reward information is available at [flare-foundation/fsp-rewards](https://github.com/flare-foundation/fsp-rewards/tree/main/flare) and on the [Discourse Forum](https://forum.flare.network/t/fsp-rewards-data) (see XLSX file for historical reward data). Updated every reward epoch (3.5 days), with epochs starting on Monday and Thursday.
- **Staking** reward information is available at [flare-foundation/reward-scripts](https://github.com/flare-foundation/reward-scripts/tree/main/generated-files). Updated every reward epoch (3.5 days), with epochs starting on Monday and Thursday.

Currently, claiming rewards needs to be performed manually for both [FSP rewards](https://github.com/flare-foundation/FTSO-Scaling/blob/main/scripts/rewards/README.md#claiming-rewards) and [validator staking rewards](/network/guides/using-flare-stake-tool#claiming-rewards). Automated solutions are being worked upon to simplify this process.
...
You can verify if your entity meets the required minimal conditions for FSP reward eligibility using the **[Flare Systems Explorer](https://flare-systems-explorer.flare.network/providers?tab=minimalConditions)**. Navigate to the **Minimal Conditions** tab.

- **Eligibility:** Your entity must show a **`YES`** status for a specific reward epoch to be eligible for FSP rewards for that period. The requirements themselves are defined in [FIP.10](/network/fsp/rewarding#minimal-conditions).
- **Pass System:** Meeting minimal conditions earns a "pass" (up to a maximum of 3 stored passes). These passes act as a buffer, automatically covering occasional performance dips (like missed submissions or uptime issues) in subsequent epochs, thus preventing immediate reward loss. If you fail to meet conditions and have no passes, FSP rewards for that epoch are forfeited.

Verify your entity's minimal conditions via CLI - entity address specified should be without 0x prefix:

```bash
curl "https://flare-systems-explorer-backend.flare.network/api/v0/entity?query=<entity-address>" -H "Accept: application/json" | jq -r '.results[0].entityminimalconditions'
```
</details>

<details>
<summary>Where can I find calculated rewards data?</summary>
...
- **Validator Staking Rewards:**
  ...
  Inspect your entity's validator rewards for a specific epoch via CLI:

  ```bash
  NODE_ID="<your-node-id>"
  EPOCH_ID="<reward-epoch-id>"
...
- **FSP (FTSO & FDC) Rewards:**
  ...
  Inspect your entity's FSP rewards for a specific epoch via CLI:

  ```bash
  ENTITY_ADDRESS="0x<your-entity-address>"
  EPOCH_ID="<reward-epoch-id>"
  curl -s "https://raw.githubusercontent.com/flare-foundation/fsp-rewards/refs/heads/main/flare/${EPOCH_ID}/reward-distribution-data.json" | \
    jq --arg entity_address "${ENTITY_ADDRESS}" \
    '.rewardClaims[] |
    select(.body.beneficiary == $entity_address) |
