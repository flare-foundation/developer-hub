...
Automated solutions are being worked upon to simplify this process.
...
:::info[Deploying components on multiple servers]

It is possible to split the deployment across multiple servers, but this deviation from the recommended deployment configuration will add friction to updating the FSP provider.
...
:::info[Monitoring the health of your deployment]
...
- For submissions, your log should contain:
  ...
  ⚠️ **Alert Tip:** Set an alert if there is no `Submitter ... successfully sent tx` message for 5-10 minutes
...
- For finalization verification, your log should contain:
  ...
  ⚠️ **Alert Tip:** Set an alert if there is no `Relaying finished ... message` for 5-10 minutes (note that protocol 100 is FTSOv2 and 200 is FDC)
