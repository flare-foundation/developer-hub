...
\*Updated every reward epoch, i.e. 3.5 days, with epochs starting on Monday and Thursday.

:::

:::warning[Claiming rewards]

Currently, claiming rewards needs to be performed [manually](https://github.com/flare-foundation/FTSO-Scaling/blob/main/scripts/rewards/README.md#claiming-rewards).
Automated solutions are being worked upon.

:::

:::tip[Name & Logo on explorers]
...
- **Monitoring the health of your deployment**

  A simple option is to monitor the `/ext/health` endpoint, and inspect the connectivity of your validator using this [API endpoint](https://build.avax.network/docs/api-reference/p-chain/api#platformgetcurrentvalidators) and checking uptime/connected values.
  For more advanced monitoring, Avalanche has a list of [Grafana dashboards](https://support.avax.network/en/articles/6159074-grafana-dashboards) that can be used for Flare.

  To check FDC health, you can query the FDC verifier by making a `GET` request on `http://localhost:9800/verifier/{chain}/health`

  To check FTSO health, you can inspect the logs to make sure everything is running as expected. First, check if you are passing minimal conditions in the epoch:
  ...
  For submissions, your log should contain (**Tip:** Set an alert if there is no `Submitter ... successfully sent tx` for 5 - 10 minutes):
  ...
  And finalizations (**Tip:** Set an alert if there is no `Relaying finished ...` for 5-10 mins, note that protocol 200 is FDC so this would be checking FDC submissions as well):
...
- **Get your name and logo listed on explorers**
