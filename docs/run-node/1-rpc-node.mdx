...
<Tabs groupId="network" block>
  ...
  <TabItem value="songbird" label="Songbird Canary-Network">
    ...
    ```bash
    git clone --branch v1.9.0 https://github.com/flare-foundation/go-flare.git
    cd go-flare/avalanchego
    chmod +x scripts/build.sh
    ./scripts/build.sh
    ```
...
<Tabs groupId="network" block>
  <TabItem value="flare" label="Flare Mainnet" default>
    ...
    Use the Docker image at [go-flare](https://hub.docker.com/layers/flarefoundation/go-flare/v1.9.0/images/sha256-b12f32b86e71f5d954de820abe2bf3a69b6ae46a6ae94853e5dd3d7d1162e32b?context=explore). The **Overview** tab in the repository linked explains the configurable parameters.

    Download and start the container:

    ```bash
    docker pull flarefoundation/go-flare:v1.9.0
    docker run -d --name flare-observer -e AUTOCONFIGURE_BOOTSTRAP="1" -e NETWORK_ID="flare" -e AUTOCONFIGURE_PUBLIC_IP="1" -e AUTOCONFIGURE_BOOTSTRAP_ENDPOINT="https://flare-bootstrap.flare.network/ext/info" -v /mnt/db:/app/db -v /opt/flare/conf:/app/conf/C -v /opt/flare/logs:/app/logs -p 0.0.0.0:9650:9650 -p 0.0.0.0:9651:9651 flarefoundation/go-flare:v1.9.0
    ```
  ...
  <TabItem value="coston2" label="Flare Testnet Coston2">
    ...
    ```bash
    docker pull flarefoundation/go-flare:v1.9.0
    docker run -d --name coston2-observer -e AUTOCONFIGURE_BOOTSTRAP="1" -e NETWORK_ID="costwo" -e AUTOCONFIGURE_PUBLIC_IP="1" -e AUTOCONFIGURE_BOOTSTRAP_ENDPOINT="https://coston2-bootstrap.flare.network/ext/info" -v /mnt/db:/app/db -v /opt/flare/conf:/app/conf/C -v /opt/flare/logs:/app/logs -p 0.0.0.0:9650:9650 -p 0.0.0.0:9651:9651 flarefoundation/go-flare:v1.9.0
    ```
  ...
  <TabItem value="songbird" label="Songbird Canary-Network">
    ...
    ```bash
    docker pull flarefoundation/go-flare:v1.9.0
    docker run -d --name songbird-observer -e AUTOCONFIGURE_BOOTSTRAP="1" -e NETWORK_ID="songbird" -e AUTOCONFIGURE_PUBLIC_IP="1" -e AUTOCONFIGURE_BOOTSTRAP_ENDPOINT="https://songbird.flare.network/ext/info" -v /mnt/db:/app/db -v /opt/flare/conf:/app/conf/C -v /opt/flare/logs:/app/logs -p 0.0.0.0:9650:9650 -p 0.0.0.0:9651:9651 flarefoundation/go-flare:v1.9.0
    ```
  ...
  <TabItem value="coston" label="Songbird Testnet Coston">
    ...
    ```bash
    docker pull flarefoundation/go-flare:v1.9.0
    docker run -d --name coston-observer -e AUTOCONFIGURE_BOOTSTRAP="1" -e NETWORK_ID="coston" -e AUTOCONFIGURE_PUBLIC_IP="1" -e AUTOCONFIGURE_BOOTSTRAP_ENDPOINT="https://coston-bootstrap.flare.network/ext/info" -v /mnt/db:/app/db -v /opt/flare/conf:/app/conf/C -v /opt/flare/logs:/app/logs -p 0.0.0.0:9650:9650 -p 0.0.0.0:9651:9651 flarefoundation/go-flare:v1.9.0
    ```
  ...
  <TabItem value="flare" label="Flare Mainnet" default>
    ...
    services:
      observer:
        ...
        image: flarefoundation/go-flare:v1.9.0
        restart: on-failure
        environment:
