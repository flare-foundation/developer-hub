...
An RPC node (also sometimes referred to as an observer node) is responsible for servicing application requests. Unlike [validator nodes](./validator-node), RPC nodes operate outside the network and do not influence consensus or block production. This guide will walk you through deploying an RPC node for Flare.
...
<Tabs groupId="network" block>
  ...
  <TabItem value="songbird" label="Songbird Canary-Network">
    ...
    ```bash
    git clone --branch v0.6.6-songbird https://github.com/flare-foundation/go-flare.git
    cd go-flare/avalanchego
    chmod +x scripts/build.sh
    ./scripts/build.sh
    ```
...
<Tabs groupId="network" block>
  ...
  <TabItem value="songbird" label="Songbird Canary-Network">
    ...
    Use the Docker image at [go-flare:v0.6.6-songbird](https://hub.docker.com/layers/flarefoundation/go-flare/v0.6.6-songbird/images/sha256-86b90c8aeb91d5fe6efaeebf71ddd028405ff99bf4ed7c31e992dd170f063363?context=explore). The **Overview** tab in the repository linked explains the configurable parameters.

    Download and start the container:

    ```bash
    docker pull flarefoundation/go-flare:v0.6.6-songbird
    docker run -d --name songbird-observer -e AUTOCONFIGURE_BOOTSTRAP="1" -e NETWORK_ID="songbird" -e AUTOCONFIGURE_PUBLIC_IP="1" -e AUTOCONFIGURE_BOOTSTRAP_ENDPOINT="https://songbird.flare.network/ext/info" -v /mnt/db:/app/db -v /opt/flare/conf:/app/conf/C -v /opt/flare/logs:/app/logs -p 0.0.0.0:9650:9650 -p 0.0.0.0:9651:9651 flarefoundation/go-flare:v0.6.6-songbird
    ```
  ...
  <TabItem value="coston" label="Songbird Testnet Coston">
    ...
    Use the Docker image at [go-flare:v0.6.6-coston](https://hub.docker.com/layers/flarefoundation/go-flare/v0.6.6-coston/images/sha256-b88cf18baf5300a4e15b630dd6de669e48ab316a22cffd370961cdd68226ef0a?context=explore). The **Overview** tab in the repository linked explains the configurable parameters.

    Download and start the container:

    ```bash
    docker pull flarefoundation/go-flare:v0.6.6-coston
    docker run -d --name coston-observer -e AUTOCONFIGURE_BOOTSTRAP="1" -e NETWORK_ID="coston" -e AUTOCONFIGURE_PUBLIC_IP="1" -e AUTOCONFIGURE_BOOTSTRAP_ENDPOINT="https://coston.flare.network/ext/info" -v /mnt/db:/app/db -v /opt/flare/conf:/app/conf/C -v /opt/flare/logs:/app/logs -p 0.0.0.0:9650:9650 -p 0.0.0.0:9651:9651 flarefoundation/go-flare:v0.6.6-coston
    ```
  ...
  <TabItem value="songbird" label="Songbird Canary-Network">
    ...
    services:
      observer:
        ...
        image: flarefoundation/go-flare:v0.6.6-songbird
        restart: on-failure
        environment:
          - AUTOCONFIGURE_BOOTSTRAP=1
          - NETWORK_ID=songbird
