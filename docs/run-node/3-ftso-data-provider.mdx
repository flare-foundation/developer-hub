...
The protocol operation uses normalized weights, and the delegation account should have at least 150 WC2FLR on Flare Testnet Coston2 (or WCFLR on Songbird Testnet Coston) to obtain a non-zero vote power.
...
Account registration can be performed manually using the `EntityManager` contract on:

- [Flare Testnet Coston2](https://coston2-explorer.flare.network/address/0xB16C9bd0783D153a32029d3D8473532DE1A80A79/write-contract#address-tabs)
- [Songbird Canary-Network](https://songbird-explorer.flare.network/address/0x46C417D0760198E94fee455CE0e223262a3D0049/write-contract#address-tabs)
- [Songbird Testnet Coston](https://coston-explorer.flare.network/address/0x60A848E5Da796D741e559c170E851FC813061217/write-contract#address-tabs)

The required functions on the `EntityManager` contract are:

- `proposeSubmitAddress` and `confirmSubmitAddressRegistration`
- `proposeSubmitSignaturesAddress` and `confirmSubmitSignaturesAddressRegistration`
- `proposeSigningPolicyAddress` and `confirmSigningPolicyAddressRegistration`
- `proposeDelegationAddress` and `confirmDelegationAddressRegistration`

The required contract invocation steps can also be found in the [register-entities.ts](https://github.com/flare-foundation/flare-smart-contracts-v2/blob/main/deployment/tasks/register-entities.ts#L33) deployment task.
...
| `FAST_UPDATES_ACCOUNTS`              | Comma separated private keys of fast updates submission accounts.                                                                                                                                                                         |
| `FAST_UPDATES_SORTITION_PRIVATE_KEY` | Private key that was registered with the network (see [Generate sortition key](#generate-sortition-key)).                                                                                                                                 |
...
If you need to wipe the database you will need to remove the volume manually. When the codebase is changed
