...
The protocol operation uses normalized weights, and the delegation account should have at least 150 wrapped native tokens to obtain a non-zero vote power. Wrapped native tokens are - WFLR on Flare Mainnet, WC2FLR on Flare Testnet Coston2, WSGB on Songbird Canary-Network, and WCFLR on Songbird Testnet Coston.
...
- (optional) [`proposeDelegationAddress`](/network/solidity-reference/IEntityManger#proposedelegationaddress)
