---
sidebar_position: 1
slug: /
title: Flare Developer Hub ☀️
sidebar_label: Home
description: Build on Flare, the EVM-compatible Layer 1 engineered for a data-rich, interconnected future
keywords:
  [
    flare-network,
    blockchain,
    evm-compatible,
    solidity,
    layer-1,
    data-rich,
    cross-chain-interoperability,
    smart-contracts,
    ftso,
    flare-time-series-oracle,
    fdc,
    flare-data-connector,
    fassets,
    defi,
    ai,
    identity,
    tradfi,
    oracle,
  ]
---

import ThemedImage from "@theme/ThemedImage";
import useBaseUrl from "@docusaurus/useBaseUrl";
import DocCard from "@theme/DocCard";

Flare is the EVM-compatible Layer 1 engineered for a data-rich, interconnected future.
Uniquely equipped with enshrined data protocols, trust-minimized interoperability, and support for complex computation, Flare enables intelligent, chain-agnostic applications bridging DeFi, AI, identity, and traditional finance with real-world utility.

## Getting Started

<div className="row">
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Deploy your first contract",
        href: "/network/getting-started",
        description:
          "Deploy a smart contract on Flare using only your browser.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Use Hardhat or Foundry with Flare",
        href: "/network/guides/hardhat-foundry-starter-kit",
        description:
          "Build and deploy using the Hardhat or Foundry Starter Kits.",
        docId: undefined,
      }}
    />
  </div>
</div>

| Network      | Flare Mainnet                                    | Flare Testnet Coston2                                                            |
| :----------- | :----------------------------------------------- | :------------------------------------------------------------------------------- |
| **RPC**      | `https://flare-api.flare.network/ext/C/rpc`      | `https://coston2-api.flare.network/ext/C/rpc`                                    |
| **Chain ID** | `14`                                             | `114`                                                                            |
| **Explorer** | [`https://flarescan.com`](https://flarescan.com) | [`https://coston2.testnet.flarescan.com`](https://coston2.testnet.flarescan.com) |
| **Faucet**   | N/A                                              | Request C2FLR from the [Coston2 Faucet](https://faucet.flare.network/coston2)    |

## Integrate Flare's protocols

<div className="row">
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Flare Time Series Oracle (FTSOv2)",
        href: "/ftso/getting-started",
        description:
          "Easily integrate fast, secure, and decentralized price feeds.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      tag="Coming Soon"
      item={{
        type: "link",
        label: "Flare Data Connector (FDC)",
        href: "/fdc/getting-started",
        description:
          "Reliably access external blockchain events and real-world APIs.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "FAssets",
        href: "/fassets/overview",
        description: "Securely explore XRPfi, BTCfi and DOGEfi.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "🚧 Protocol Managed Wallets",
        href: "https://flare.network/news/flare-vision",
        description: "(In development) Unlock cross-chain execution with TEEs.",
        docId: undefined,
      }}
    />
  </div>
</div>

## Build faster with ecosystem tools

<div className="row">
  <div className="col col--3 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Wallet SDKs",
        href: "/network/developer-tools#wallet-sdks",
        description: "Turnkey, Wagmi, Web3Auth, RainbowKit, Etherspot.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--3 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Bridges",
        href: "/network/developer-tools#bridges",
        description: "LayerZero, Stargate, zkBridge.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--3 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Indexers",
        href: "/network/developer-tools#indexers",
        description: "Goldsky, sqd, SubQuery.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--3 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "OFTs",
        href: "/network/developer-tools#ofts",
        description: "USD₮0, flrETH, USDC.e, WETH, USDT.",
        docId: undefined,
      }}
    />
  </div>
</div>

## Understand the architecture

Gain a deep understanding of the core concepts that differentiate Flare from other blockchains.
Its native data protocols - [Flare Time Series Oracle](/ftso/overview) and [Flare Data Connector](/fdc/overview) - are enshrined directly into the core [Flare Systems Protocol](/network/fsp), inheriting the full economic security of the Flare network.

<ThemedImage
  alt="Diagram of Flare's architecture, illustrating the Flare Systems Protocol with its enshrined data protocols including the Flare Time Series Oracle and Flare Data Connector."
  sources={{
    light: useBaseUrl("img/flare_architecture_light.svg"),
    dark: useBaseUrl("img/flare_architecture_dark.svg"),
  }}
/>

## Contribute to Flare

<div className="row">
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Run an RPC Node",
        href: "/run-node#rpc-node",
        description: "Run a Flare node using docker or from source.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Run a Validator Node",
        href: "/run-node#validator-node",
        description: "Register your Flare node as a validator.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Run a Flare Entity",
        href: "/run-node#flare-entity",
        description:
          "Provide data for Flare's enshrined FTSO and FDC protocols.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Contribute to Flare AI Kit",
        href: "https://github.com/flare-foundation/flare-ai-kit",
        description: "Build the future of Verifiable AI with TEEs on Flare.",
        docId: undefined,
      }}
    />
  </div>
</div>

## Getting Support

<div className="row">
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Get technical support",
        href: "https://flare.network/resources/developer-support",
        description: "Consult with Flare experts for dedicated support.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Chat with the community",
        href: "https://t.me/FlareNetwork",
        description: "Join the conversation in the Flare Telegram channel.",
        docId: undefined,
      }}
    />
  </div>
</div>
