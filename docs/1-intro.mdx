---
sidebar_position: 1
slug: /
title: Flare Developer Hub
sidebar_label: Home
description: Official documentation for Flare — tutorials, SDKs, API docs, and agent tools for building on Flare's enshrined data protocols.
keywords:
  [
    flare-network,
    flare-docs,
    flare-documentation,
    blockchain,
    evm-compatible,
    solidity,
    layer-1,
    data-rich,
    cross-chain-interoperability,
    smart-contracts,
    ftso,
    flare-time-series-oracle,
    fdc,
    flare-data-connector,
    fassets,
    defi,
    ai,
    oracle,
  ]
---

import ThemedImage from "@theme/ThemedImage";
import useBaseUrl from "@docusaurus/useBaseUrl";
import DocCard from "@theme/DocCard";
import CopyButton from "@site/src/components/CopyButton";
import ToolCardGrid from "@site/src/components/ToolCardGrid";

Start building on Flare, the EVM-compatible Layer 1 built for data-intensive, interoperable applications.
Flare Developer Hub is your central resource for tutorials, SDKs, and API docs.
Build with Flare's enshrined data protocols, fast finality, and soon, verifiable compute with TEEs.

## Getting Started

<div className="row">
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Deploy your first contract",
        href: "/network/getting-started",
        description:
          "Deploy a smart contract on Flare using only your browser.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Use Hardhat or Foundry with Flare",
        href: "/network/guides/hardhat-foundry-starter-kit",
        description:
          "Build and deploy using the Hardhat or Foundry Starter Kits.",
        docId: undefined,
      }}
    />
  </div>
</div>

| Network             | Flare Mainnet                                                                                                                                                                | Flare Testnet Coston2                                                                                                                                                            |
| :------------------ | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **RPC**             | <code>flare-api.flare.network/ext/C/rpc</code> <CopyButton textToCopy="https://flare-api.flare.network/ext/C/rpc" />                                                         | <code>coston2-api.flare.network/ext/C/rpc</code> <CopyButton textToCopy="https://coston2-api.flare.network/ext/C/rpc" />                                                         |
| **Alternative RPC** | <code>stylish-light-theorem.flare-mainnet.quiknode.pro/ext/bc/C/rpc</code> <CopyButton textToCopy="https://stylish-light-theorem.flare-mainnet.quiknode.pro/ext/bc/C/rpc" /> | <code>falling-skilled-uranium.flare-coston2.quiknode.pro/ext/bc/C/rpc</code> <CopyButton textToCopy="https://falling-skilled-uranium.flare-coston2.quiknode.pro/ext/bc/C/rpc" /> |
| **Chain ID**        | <code>14</code> <CopyButton textToCopy="14" />                                                                                                                               | <code>114</code> <CopyButton textToCopy="114" />                                                                                                                                 |
| **Explorer**        | [`https://flare-explorer.flare.network`](https://flare-explorer.flare.network)                                                                                               | [`https://coston2-explorer.flare.network`](https://coston2-explorer.flare.network)                                                                                               |
| **Faucet**          | N/A                                                                                                                                                                          | Request C2FLR, FXRP and USDT0 from the [Coston2 Faucet](https://faucet.flare.network/coston2)                                                                                    |

Additional configuration and API resources are detailed on the [Network](/network/overview) page.

## Integrate Flare's protocols

<div className="row">
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      tag="New"
      item={{
        type: "link",
        label: "Flare Smart Accounts",
        href: "/smart-accounts/overview",
        description: "Account abstraction for XRPL users on Flare.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      tag="New"
      item={{
        type: "link",
        label: "FXRP",
        href: "/fxrp/overview",
        description: "Use XRP on Flare via trustless FAssets.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "FAssets",
        href: "/fassets/overview",
        description: "Securely explore XRPfi, BTCfi and DOGEfi.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Flare Time Series Oracle (FTSOv2)",
        href: "/ftso/getting-started",
        description:
          "Easily integrate fast, secure, and decentralized price feeds.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Flare Data Connector (FDC)",
        href: "/fdc/getting-started",
        description:
          "Reliably access external blockchain events and real-world APIs.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "🚧 Protocol Managed Wallets",
        href: "https://flare.network/news/flare-vision",
        description: "(In development) Unlock cross-chain execution with TEEs.",
        docId: undefined,
      }}
    />
  </div>
</div>

## AI tools

Any documentation page is available as agent-ready Markdown: append `.md` to the page URL
(for example, `https://dev.flare.network/network/overview.md`) or use the
**Copy page** button in the table of contents sidebar.
The full corpus index is at [`llms.txt`](pathname:///llms.txt).

<ToolCardGrid
  colClassName="col--6"
  items={[
    {
      label: "Flare AI Skills",
      href: "/network/guides/flare-ai-skills",
      description:
        "Flare protocol AI skills for Claude Code, Cursor and other agents.",
    },
    {
      label: "MCP Server",
      href: "/network/guides/flare-developer-hub-mcp-server",
      description: "Connect AI tools directly to Flare data and services.",
    },
    {
      label: "llms.txt",
      href: "pathname:///llms.txt",
      description:
        "Machine-readable index of all documentation for LLM ingestion.",
    },
  ]}
/>

## Ecosystem tools

<ToolCardGrid
  items={[
    {
      label: "Wallet SDKs",
      href: "/network/developer-tools#wallet-sdks",
      description:
        "Turnkey, Wagmi, MetaMask Embedded Wallets, RainbowKit, Etherspot.",
    },
    {
      label: "Bridges",
      href: "/network/developer-tools#bridges",
      description: "LayerZero, Stargate, zkBridge.",
    },
    {
      label: "Indexers",
      href: "/network/developer-tools#indexers",
      description: "Goldsky, sqd, SubQuery.",
    },
    {
      label: "OFTs",
      href: "/network/developer-tools#ofts",
      description: "USD₮0, flrETH, USDC.e, WETH, USDT.",
    },
  ]}
/>

## Understand the architecture

Gain a deep understanding of the core concepts that differentiate Flare from other blockchains.
Its native data protocols, [Flare Time Series Oracle](/ftso/overview) and [Flare Data Connector](/fdc/overview), are enshrined directly into the core [Flare Systems Protocol](/network/fsp), inheriting the full economic security of the network.

<ThemedImage
  alt="Flare Systems Protocol with enshrined FTSO and FDC."
  sources={{
    light: useBaseUrl("img/docs/flare-architecture.light.svg"),
    dark: useBaseUrl("img/docs/flare-architecture.dark.svg"),
  }}
/>

## Contribute to Flare

<div className="row">
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Run an RPC Node",
        href: "/run-node#rpc-node",
        description: "Run a Flare node using docker or from source.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Run a Validator Node",
        href: "/run-node#validator-node",
        description: "Register your Flare node as a validator.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Run a Flare Entity",
        href: "/run-node#flare-entity",
        description:
          "Provide data for Flare's enshrined FTSO and FDC protocols.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Contribute to Flare AI Kit",
        href: "https://github.com/flare-foundation/flare-ai-kit/issues",
        description: "Build the future of Verifiable AI with TEEs on Flare.",
        docId: undefined,
      }}
    />
  </div>
</div>

## Getting Support

<div className="row">
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Get technical support",
        href: "https://flare.network/resources/technical-support",
        description: "Consult with Flare experts for dedicated support.",
        docId: undefined,
      }}
    />
  </div>
  <div className="col col--6 margin-bottom--lg">
    <DocCard
      item={{
        type: "link",
        label: "Chat with the community",
        href: "https://t.me/FlareNetwork",
        description: "Join the conversation in the Flare Telegram channel.",
        docId: undefined,
      }}
    />
  </div>
</div>
