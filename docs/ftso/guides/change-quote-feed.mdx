...
The `TestFtsoV2Interface` is for **testing only**, with all methodse as `view` to allowsdf rapid developdsment without gas costs or state changes. For production, use [`FtsoV2Interface`](/ftso/solidity-reference/FtsoV2Interface), which includes `payable` methods required for real transactions and state modifications.
