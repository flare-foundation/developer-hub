...
2. **FastUpdater Contract Address:** The address for the `FastUpdater` contract is different for each network. You can query this address by calling the `FlareContractRegistry`, which has the same address across all networks. Examples for this are given in the language guides using [Python](/guides/flare-for-python-developers#make-query), [Rust](/guides/flare-for-rust-developers#make-query), [JavaScript](/guides/flare-for-javascript-developers#make-query), and [Go](/guides/flare-for-go-developers#make-query).
