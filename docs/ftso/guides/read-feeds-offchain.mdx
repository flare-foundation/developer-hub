...
This guide contains code examples that demonstrate how to read FTSOv2 feeds off-chain using different programming languages. To read a block-latency feed off-chain you need three pieces of information:
...
2. **Contract Address:** The address for the `FastUpdater` contract is different for each network. You can either:

   - Find the `FastUpdater` contract address on the [Solidity Reference](../solidity-reference) page,
...
3. **Feed Indexes:** The feeds you want to read are uniquely identified by their index. A list of feed indexes is given on the [Block-Latency Feeds](../feeds) page.
...
```rust title="src/bin/ftsov2_consumer.rs"
// THIS IS EXAMPLE CODE. DO NOT USE THIS CODE IN PRODUCTION.
...
async fn main() -> Result<()> {
    ...
    // Set up contract instance
    let ftsov2 = FastUpdater::new(ftso_address, provider);
    // Fetch current feeds
    let FastUpdater::fetchCurrentFeedsReturn {
        _feeds,
        _decimals,
        _timestamp,
...
With Go, you need to manually fetch the contract's ABI and generate the Go bindings. Copy the Contract ABI for [FastUpdater on Flare Testnet Coston2](../solidity-reference) and paste it into a file named `FastUpdater.abi`, located in the root of your project, i.e. same level as `go.mod`. Then using [abigen](https://geth.ethereum.org/docs/tools/abigen), generate the Go bindings.
...
func FtsoV2Consumer() {
  // FastUpdater address (Flare Testnet Coston2)
  address := common.HexToAddress("0x58fb598EC6DB6901aA6F26a9A2087E9274128E59")
