...
This guide is for developers who want to build an application using FTSOv2 in Foundry. In this guide, using Foundry, you will learn how to:
...
- Compile your contract using Foundry [forge](https://book.getfoundry.sh/reference/forge/), and deploy it to Flare Testnet Coston2.

- Interact with your onchain contract using Foundry [cast](https://book.getfoundry.sh/reference/cast/).
...
2. Create a file `contracts/remappings.txt`, and add the following remappings to ensure Foundry can locate the installed packages:
   ...
   ```plaintext title="contracts/remappings.txt"
