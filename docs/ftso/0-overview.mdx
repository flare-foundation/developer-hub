...
- **Cost-effective.** Block-latency feeds in FTSOv2 are free to use onchain.
  Feeds from Scaling are also free to query and verify locally, with minimal gas costs for onchain verification.
...
<ThemedImage
  ...
  sources={{
    light: useBaseUrl(
      "img/docs/ftso/overview/ref-fast-comparison-social.light.svg",
    ),
    dark: useBaseUrl(
      "img/docs/ftso/overview/ref-fast-comparison-social.dark.svg",
    ),
  }}
/>
...
The expected sample size (average) is `1` by default, and data providers have no control over, nor knowledge of, when they will be selected.
...
To maintain security, the seed value itself evolves pseudo-randomly.
This approach balances security and randomness, preventing adversaries from influencing the selection process.
...
<ThemedImage
  ...
  sources={{
    light: useBaseUrl(
      "img/docs/ftso/overview/ftso-update-architecture.light.svg",
    ),
    dark: useBaseUrl(
      "img/docs/ftso/overview/ftso-update-architecture.dark.svg",
    ),
  }}
/>
...
Data providers are rewarded when the block-latency feeds remain within the primary reward band (the interquartile range band) of the anchor feed selected to determine rewards for that voting epoch.
