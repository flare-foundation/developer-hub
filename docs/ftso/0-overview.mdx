...
- **Cost-effective.** Block-latency feeds in FTSOv2 are completely free to query, without even gas costs. Feeds from Scaling are also free to query and verify locally, with minimal gas costs for onchain verification.
