...
`IFlareContractRegistry` is a core interface that provides access to the addresses of all key contracts within the Flare system. It is deployed at a fixed address - `0xaD67FE66660Fb8dFE9d6b1b4240d8650e30F6019` on all networks, with addresses updated through governance. This registry serves as the recommended entry point for interacting with contracts across the Flare ecosystem.

For a more user-friendly experience, you can use the `ContractLibrary` available in the periphery package. This library wraps the `IFlareContractRegistry` and simplifies interaction with the system.
...
```solidity title="FeeCalculator.sol"
// SPDX-License-Identifier: MIT
pragma solidity >=0.7.6 <0.9;
