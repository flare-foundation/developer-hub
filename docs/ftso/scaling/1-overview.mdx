...
<ThemedImage
  ...
  sources={{
    light: useBaseUrl("img/docs/ftso/overview/voting-epoch.light.svg"),
    dark: useBaseUrl("img/docs/ftso/overview/voting-epoch.dark.svg"),
  }}
/>
...
- **Finalization**: Once a sufficient weight of signatures for the same Merkle root is collected, a randomly selected set of providers (or any other entity in case of a failure), can collect and submit them onchain for verification.
...
5. **Accumulate weights:** Move through the sorted list, adding weights until the cumulative total ≥ $M$.
