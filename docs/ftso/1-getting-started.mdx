...
You can utilize FTSOv2 to connect your smart contracts with real-world data feeds. FTSOv2 harnesses Flare's network of 100 independent data providers to fetch off-chain data and deliver it on-chain. This section demonstrates how to consume FTSOv2's block-latency feeds on Flare using an on-chain Solidity contract.
...
{/* prettier-ignore */}
1. [Open the contract in Remix](https://remix.ethereum.org/#version=soljson-v0.8.25+commit.b61c2a91.js&optimize=true&runs=200&gist=d6181b71e7932701efbc6c3c78898f5a)
...
The first `feedValues` array will contain the current integer values of FLR/USD, BTC/USD and ETH/USD in the same order as the feed indexes were given. The second `decimals` array will contain the decimal places for the corresponding feeds.
