...
const name = "flare-devhub";
const version = "1.0.0";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const DOCS_BASE_URL = "https://dev.flare.network";
const MCP_DOCS_PATH = path.join(__dirname, "build/mcp/docs.json");
const MCP_INDEX_PATH = path.join(__dirname, "build/mcp/search-index.json");
const SITE_SEARCH_INDEX_PATH = path.join(__dirname, "build/search-index.json");
...
function searchSiteIndex(searchIndex, query, limit = 5) {
  ...
  for (const batch of searchIndex) {
    ...
    for (const hit of hits) {
      ...
      if (heading) matchingHeadings.push(heading);
...
function createFallbackServer() {
      ...
      if (!searchIndex) {
        return {
          content: [
            {
              type: "text",
              text: "docs_search is unavailable locally because no search index was found. Run `npm run build` to generate `build/search-index.json` or `build/mcp/search-index.json`.",
            },
          ],
          isError: true,
        };
  ...
  return {
    mode: searchIndex ? "fallback-search-index" : "fetch-only",
...
function createServer() {
  if (fs.existsSync(MCP_DOCS_PATH) && fs.existsSync(MCP_INDEX_PATH)) {
    return {
...
http
  ...
  .listen(3456, () => {
    ...
    if (mode === "fallback") {
      console.warn(
        "build/mcp artifacts were not found. Falling back to build/search-index.json for docs_search and runtime page fetching for docs_fetch.",
      );
    }
  });
