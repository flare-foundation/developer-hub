...
<Tabs groupId="python">
  <TabItem value="poetry" label="poetry" default>
    ```bash 
    poetry run python -c "from web3 import Web3; w3 = Web3(); acc =
    w3.eth.account.create(); print(f'private key={w3.to_hex(acc.key)}, account=
    {acc.address}')" 
    ```
  ...
  <TabItem value="pip" label="pip">
    ```bash 
    python -c "from web3 import Web3; w3 = Web3(); acc =
    w3.eth.account.create(); print(f'private key={w3.to_hex(acc.key)}, account=
    {acc.address}')" 
    ```
