...
<Tabs groupId="network">
  <TabItem value="coston2" label="Coston2 Testnet" default>
    ...
    async def main():
        ...
        print(f"WNat address: {res}")  # WNat address: 0xC67DCE33D7A8efA5FfEB961899C73fe01bCe9273
  ...
  <TabItem value="flare" label="Flare Mainnet">
    ...
    async def main():
        ...
        contract = w3.eth.contract(address=registry_addr, abi=abi)
        res = await contract.functions.getContractAddressByName("WNat").call()
        print(f"WNat address: {res}")  # WNat address: 0x1D80c49BbBCd1C0911346656B529DF9E5c2F783d
...
For this example, you can write a simple contract to query the FTSOv2 feed data. You can reuse the `FtsoV2FeedConsumer.sol` contract from the [FTSOv2 Sample Contract](../docs/ftso/getting-started#sample-contract) guide. Save the `.sol` file in the same folder as your Python script.
