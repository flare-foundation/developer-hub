...
You can now run the `deploy_contract.rs` script to deploy the contract. The contract address will be printed once the deployment is successful. You can check the contract address on a Flare block explorer, linked on the [Network Configuration](../docs/flare/configuration#configuration) page.
