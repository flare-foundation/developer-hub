...
authors: [dineshpinto]
tags: [rust, quickstart, alloy-rs, solc]
---
...
- Query a contract on Flare using [alloy-rs](https://github.com/alloy-rs), an async library designed to interact with Ethereum-like chains.
...
eyre = "0.6.12"
alloy = { git = "https://github.com/alloy-rs/alloy", rev = "ec3a00dc00d68014565e39350b17cd2a56d0ceeb", features = ["contract", "providers", "transport-http", "network", "signers", "signer-wallet"]}
```
...
This will generate a `FtsoV2FeedConsumer.json` file with the contract's ABI and bytecode. Things get a bit annoying here, as the Rust solc importer expects a specific format for the JSON, which is not the same as the output from the Solidity compiler.

### Convert to Rust format

Remove the two top-level JSON fields i.e. "contracts" and "FtsoV2FeedConsumer", the JSON should look like:
...
- For testnet, you can use the testnet faucets on the [Network Configuration](../docs/flare/configuration#configuration) page to get some testnet `C2FLR` tokens.
