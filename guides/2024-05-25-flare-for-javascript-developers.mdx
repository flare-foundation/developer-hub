...
You need to connect to testnet or mainnet via an RPC, any RPC listed on the [Network Configuration](../docs/flare/network#configuration) page will work. For this guide, you can use the Ankr RPC.
...
- For testnet, you can use the testnet faucets on the [Network Configuration](../docs/flare/network#configuration) page to get some testnet C2FLR.
...
You can now run the `deployContract.js` script to deploy the contract. The contract address will be printed once the deployment is successful. You can check the contract address on a Flare Blockchain Explorer, linked on the [Network Configuration](../docs/flare/network#configuration) page.

Congratulations! You have now successfully deployed a contract on Flare using Python.

:::tip[What's next?]
Check out the [Solidity Reference](/docs/ftso/solidity-reference/IFastUpdater) for the Flare Time-Series Oracle (FTSOv2), to see what all on-chain functions you can interact with using JavaScript.
:::
