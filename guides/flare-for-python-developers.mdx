...
In this guide, using Python you will:

- Learn how to query a contract on Flare using [web3.py](https://web3py.readthedocs.io/en/stable/), a library for interacting with Ethereum-like chains.
- Learn how to compile a Solidity contract using [py-solc-x](https://solcx.readthedocs.io/en/latest/), a wrapper around the Solidity compiler.
- Learn how to deploy a contract on Flare.
...
if __name__ == "__main__":
    ...
    # Versions newer than 0.8.25 may work, but have not been tested
    solc_version = "0.8.25"
...
if __name__ == "__main__":
    ...
    fast_updates_feed_consumer = w3.eth.contract(
        abi=contract_interface["abi"],
        bytecode=contract_interface["evm"]["bytecode"]["object"],
    )

    # Build transaction
    tx = fast_updates_feed_consumer.constructor().build_transaction(
        {
            "from": account,
...
You can now run the `deploy_contract.py` script to deploy the contract. The contract address will be printed once the deployment is successful. You can make sure the deployment was successful by checking the contract address on a Block Explorer, whose links can be found on the Network Configuration](../docs/flare/configuration#configuration) page.
