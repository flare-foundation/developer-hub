...
description: Starter kits for integrating Flare with Hardhat and Foundry.
keywords:
...
This guide provides a starter kit for integrating Flare with Hardhat and Foundry. The starter kit includes a basic setup for configuring Hardhat and Foundry, along with examples on how to compile, test and deploy smart contracts on Flare.
...
Replace `<RPC_URL>` with the RPC endpoint of the network you are deploying to. A list of RPC endpoints for Flare networks can be found in Flare [Network Configuration](/docs/network#configuration).
