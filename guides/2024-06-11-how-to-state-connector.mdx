...
Once the attestation is requested you can calculate the `roundId` - The State Connector round that was in the collect phase the moment the attestation was requested. You now have to wait for the corresponding Reveal round to end to be able to get the proof. This takes between 3 and 4.5 minutes.
...
Let's automate this process and create a script, that creates a transaction on external chain and immediately try to see what request you would get from the state connector.
